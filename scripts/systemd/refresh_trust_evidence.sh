...
if [[ "${RUSTYNET_BIN}" != /* ]]; then
  echo "[trust-refresh] RUSTYNET_BIN must be an absolute path: ${RUSTYNET_BIN}" >&2
  exit 1
fi
if [[ ! -x "${RUSTYNET_BIN}" ]]; then
  echo "[trust-refresh] required executable not found: ${RUSTYNET_BIN}" >&2
  exit 1
fi
...
if [[ "${bin_uid}" != "0" ]]; then
  echo "[trust-refresh] rustynet binary must be root-owned: ${RUSTYNET_BIN}" >&2
  exit 1
fi
...
if [[ "${bin_mode}" =~ ^[0-9]{3,4}$ ]]; then
  ...
  if (( (10#${group_digit} & 2) != 0 || (10#${other_digit} & 2) != 0 )); then
    echo "[trust-refresh] rustynet binary must not be group/world writable: ${RUSTYNET_BIN} (${mode3})" >&2
    exit 1
  fi
else
  echo "[trust-refresh] unable to validate binary mode: ${RUSTYNET_BIN}" >&2
  exit 1
fi

exec "${RUSTYNET_BIN}" ops refresh-trust
