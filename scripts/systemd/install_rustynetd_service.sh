...
for key in \
  ...
  RUSTYNET_RECONCILE_INTERVAL_MS \
  RUSTYNET_MAX_RECONCILE_FAILURES \
  RUSTYNET_FAIL_CLOSED_SSH_ALLOW \
  RUSTYNET_FAIL_CLOSED_SSH_ALLOW_CIDRS; do
  load_existing_env_var "${key}"
done
...
if ! [[ "${WG_LISTEN_PORT}" =~ ^[0-9]+$ ]] || (( WG_LISTEN_PORT < 1 || WG_LISTEN_PORT > 65535 )); then
  echo "invalid wireguard listen port: ${WG_LISTEN_PORT} (expected 1-65535)" >&2
  exit 1
fi
...
LEGACY_PASSPHRASE_PATH="/etc/rustynet/wireguard.passphrase"
BOOTSTRAP_PASSPHRASE_PATH="/var/lib/rustynet/keys/wireguard.passphrase"
if [[ ! -f "${WG_KEY_PASSPHRASE_CREDENTIAL_BLOB_PATH}" ]]; then
  echo "missing encrypted credential blob: ${WG_KEY_PASSPHRASE_CREDENTIAL_BLOB_PATH}" >&2
...
for readonly_target in \
  ...
  "${MEMBERSHIP_OWNER_SIGNING_KEY_PATH}"; do
...
set_owner_mode_if_exists "${MEMBERSHIP_OWNER_SIGNING_KEY_PATH}" root root 0600
