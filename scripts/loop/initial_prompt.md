...
> `rustynet_live_lab_loop_prompt.md`, which is written for the agent driving the whole loop
> itself (arming Monitors, polling results, running the concurrent pipeline). Do NOT do those
> here — the script does them. Read `rustynet_live_lab_loop_prompt.md` + `AGENTS.md`/`CLAUDE.md`
> for the deep standing orders (AI-agent MCP usage, the §9 hard-decision protocol, gate
> definitions); this file governs the division of labor with the script.
...
launches live-lab runs, waits for each AI-agent MCP report, and pastes it into THIS chat with an
...
- Fetching the AI-agent report and pasting it into this chat with an action prompt.
- Detecting the run you launch next and shepherding it.
So: do NOT arm a Monitor on a run log, do NOT poll `ai_live_lab_result` yourself, do NOT
...
1. VERIFY the evidence (~30s). The AI-agent triage is UNTRUSTED — confirm the run-matrix row in
   ...
   and do your own diagnosis (the AI-agent MCP's flash tier + the grounded `ai_agent` are your
   research layer — use them, but you make the call).
...
5. RELAUNCH exactly as the pasted prompt instructs — call `ai_lab_run(...)` with the args
...
For a mac/win cell, relaunch with BOTH:
  ai_lab_run(area="<same area>", <same platform + role_platform args>,
...
On a PASS: sync docs (`ai_doc_sync`), flip the parity-matrix cell, then the prompt will
   have you pick the next unproven cell via `ai_next_live_lab_target` and launch it
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Between pasted reports you may: pre-read the next likely cell's code, fan the AI-agent MCP's
flash tier over a crate for latent fail-open paths, run local gates/fuzz, sync a stale doc, or read the parity
...
blocked, OOM, disk full) → first call `ai_reconcile_jobs` if a stale job blocks the
singleton, then call `ai_recover_lab_environment(force=true)` and poll its result with
`ai_live_lab_result`. If unrecoverable after 3 tries, `write_loop_note` the blocker and
use `ai_next_live_lab_target` + `ai_lab_run` to target a different cell — never loop
on an unrecoverable env issue.

Hard decisions (security design, architecture, which cell next) are yours — never surface them.
Use the §9 protocol in rustynet_live_lab_loop_prompt.md (project sources of truth → industry
