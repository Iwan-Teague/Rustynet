...
impl Session {
    ...
    pub(super) fn format_packet_data<'a>(&self, src: &[u8], dst: &'a mut [u8]) -> &'a mut [u8] {
        ...
        let n = {
            ...
            let sender = ChaCha20Poly1305::new_from_slice(&self.sending_key).unwrap();
            sender
                .encrypt_in_place_detached(Nonce::from_slice(&nonce), b"", &mut data[..src.len()])
                .map(|tag| {
    ...
    pub(super) fn receive_packet_data<'a>(
        ...
        let ret = {
            ...
            let receiver = ChaCha20Poly1305::new_from_slice(&self.receiving_key).unwrap();
            let tag_index = ct_len - AEAD_SIZE;
            let (ciphertext, tag_bytes) = dst[..ct_len].split_at_mut(tag_index);
            receiver
