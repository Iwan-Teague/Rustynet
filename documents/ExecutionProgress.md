...
| 2026-03-07T10:00:15Z | Phase 2 security revalidation and enrollment hardening | `crates/rustynet-control/src/lib.rs` | `cargo fmt --all`, `cargo test -p rustynet-control --all-targets --all-features`, `cargo clippy -p rustynet-control --all-targets --all-features -- -D warnings`, `cargo check --workspace --all-targets --all-features`, `cargo test --workspace --all-targets --all-features`, `cargo clippy --workspace --all-targets --all-features -- -D warnings` | Partial | Hardened persisted enrollment to require atomic DB single-use consume before token issuance; added deterministic error mapping + negative tests for missing/reused persisted credentials; switched access-token nonce issuance to high-entropy random hex. Workspace `clippy` remains red due pre-existing `rustynetd` `uninlined_format_args` warnings unrelated to Phase 2 logic. |
