...
- Phase E progress: macOS runtime stop/status/disconnect control is Rust-backed via `rustynet ops stop-runtime-service`, `rustynet ops show-runtime-service-status`, and `rustynet ops disconnect-cleanup`; `start.sh` macOS stop/status/disconnect paths now dispatch to Rust ops instead of direct `launchctl`/`pfctl`/`pkill` mutation loops.
