...
- live-lab execution and evidence: [live_lab_node_run_matrix.csv](./live_lab_node_run_matrix.csv) — the **live Rust `--node` engine ledger** (current runs append here) — with [LiveLabRunMatrix.md](./LiveLabRunMatrix.md), [live_lab_node_stage_results.csv](./live_lab_node_stage_results.csv) (exact distro/version node-stage proof), and [MeasuredEvidenceGeneration.md](./MeasuredEvidenceGeneration.md), plus the script-local function map at [scripts/e2e/README.md](../../scripts/e2e/README.md). [LiveLinuxLabOrchestrator.md](./LiveLinuxLabOrchestrator.md) and [live_lab_run_matrix.csv](./live_lab_run_matrix.csv) document/track the **legacy bash orchestrator** (frozen archive; slated for deletion under W5.7)
...
- [LiveLinuxLabOrchestrator.md](./LiveLinuxLabOrchestrator.md) — **legacy bash orchestrator** runbook (frozen; slated for deletion under W5.7 — prefer the Rust `--node` engine and its live ledger `live_lab_node_run_matrix.csv`)
