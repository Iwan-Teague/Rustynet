...
1. **Daemon constants (the 21 latent-but-inert rows):** mirror the QH-40 pattern — either add `#[cfg(target_os = "macos")]` arms at the installer roots (`/usr/local/var/rustynet/…`, `/private/var/run/…`, `/etc` → `${STATE_ROOT}/trust` equivalents) or, preferred for drift-resistance, introduce shared `default_*_path()` resolver functions like `default_state_path()` (`daemon.rs:192-194`) so every consumer resolves one function rather than a constant.
2. **CLI duplicate constants (cli:10820-10828, :10842):** delete the `rustynet-cli`-local copies and import rustynetd's (already macOS-armed for socket/state), eliminating the `/run` vs `/private/var/run` divergence.
3. **`check_macos_doctor` WG custody check (cli:17322-17332):** either point it at the real macOS custody paths (`${STATE_ROOT}/keys/wireguard.key.enc`, `${STATE_ROOT}/bootstrap/wireguard.passphrase`) or remove it; today it is dead on macOS. Keep it failing loudly, not silently-absent.
...
5. **`DEFAULT_ANCHOR_BUNDLE_PULL_TOKEN_PATH` (:216-217):** delete the dead constant (zero consumers) or wire it consistently across platforms.

## Verification status

Docs-only change (this document + its [README.md](./README.md) index entry). No production code touched, so the cargo gates are not triggered by this change; line-number citations were read directly against the `c00960a3` tree during the audit.
