...
| anchor (gossip/bundle-pull/enrollment/port-map) | ✅ | 🟡 **bundle-pull**: implemented end-to-end (launchd `com.rustynet.anchor.plist` loopback bundle-pull profile + `ops install-macos-anchor` verify-before-serve installer + `macos_service_hardening` reviewed-shape validator + `ops e2e-bootstrap-macos` seeds the bundle-pull token + live `live_macos_anchor_test` bin asserting loopback byte-for-byte / token gate / LAN-bind refused / secrets hygiene, wired into the `validate_macos_anchor_bundle_pull` vm_lab stage, FAIL-LOUD: the live result is authoritative and the dry-run plan is informational-only, never a Pass) — **deploy machinery landed 2026-06-22** (`deploy_macos_anchor_profile` stage: standalone `ops seed-macos-anchor-token`, `amend_membership_for_macos` grants `anchor.bundle_pull` when elected, bootout client daemon → `install-macos-anchor` verify-before-serve → poll listener). **BLOCKED on a real runtime bug** (run `livelab-1782135034`): the anchor daemon installs + kickstarts (exit 0) but crash-loops with `config_error 65: wg key decrypt failed — load macOS keychain passphrase failed: os secure store unavailable`, so the loopback listener never binds. The anchor daemon loaded via SSH→`launchctl bootstrap` cannot reach the macOS Keychain (unlike the bootstrap-loaded client daemon) — fix the keychain ACL / launchd session / passphrase-file fallback next; gossip/enrollment/port-map still via the shared `live_linux_anchor_test --platform macos` path | 🟠 dry-run plan contract only (`validate_windows_anchor_bundle_pull_plan_contract`) |
