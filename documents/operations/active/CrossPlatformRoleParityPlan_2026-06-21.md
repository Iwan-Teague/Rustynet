...
| blind_exit (irreversible exit) | ✅ | 🟡 **runtime code-complete, live stage deferred (corrected 2026-06-24)** — `macos_blind_exit.rs` (pf anchor builder/evaluator) + daemon invariants + the immutability gate + the mesh-egress-source CIDR bound (2026-06-24, closes the killswitch-bypass found by the pfctl-boundary review) are present and unit-tested; only the FAIL-LOUD orchestrator activation/assertion stage is missing (deferred in `active_exit.rs`). Was mislabelled ❌. | 🚫 **out of scope by design** — `main.rs:11833` hard-errors "blind_exit role is supported on Linux/macOS only"; like mobile clients, not a parity gap (corrected 2026-06-24, was ❌) |
