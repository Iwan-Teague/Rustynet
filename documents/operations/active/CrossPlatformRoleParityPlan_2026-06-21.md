...
| live role transitions (cross-OS) | ✅ (`role_switch_matrix`) | 🟡 **implemented 2026-07-04, pending live-lab proof** — `validate_macos_role_transition` stage (`--role-switch-platform macos`) drives a real `LocalOnly` admin<->client flip via `rustynet role set` + launchd bootout/bootstrap, asserts the new role + mesh peers survived the restart, then `state refresh`. `SignedMembership`-kind transitions (capability changes) are a separate follow-up. | ❌ not implemented (design in `CrossOsRoleSwitchPlan_2026-06-24.md` — the gitignored `state/` path could not persist) |
...
- **macOS remaining parity:** admin, blind_exit, relay lifecycle, exit, and
  anchor bundle-pull are live-proven; remaining macOS scope is role transitions
  plus anchor gossip/enrollment/port-map sub-surfaces.
