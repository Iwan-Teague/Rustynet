...
| blind_exit (irreversible exit) | ✅ | 🟡 **code-complete, live-run PENDING (corrected 2026-06-24)** — runtime (`macos_blind_exit.rs` pf builder/evaluator + daemon invariants + the immutability gate + the 2026-06-24 mesh-egress-source CIDR bound that closes the killswitch-bypass from the pfctl-boundary review) AND the FAIL-LOUD live stage are both present: `--blind-exit-platform macos` → `is_macos_active_blind_exit` gate → `validate_macos_blind_exit` stage (`vm_lab/mod.rs:9693`, Pass/Fail strictly from the live result, dry-run = Skipped, runs last because it factory-resets) → `exercise_macos_blind_exit_live` (`:10371`) which drives the irreversible `role set blind_exit --accept-irreversible`, asserts the pf anchor is loaded with no route-to/reply-to/dup-to, and verifies the immutability gate blocks leaving. The §9 "deferred in active_exit.rs" note was stale — it inspected only the rust-native stage; the live stage lives in the legacy `run_macos_orchestration_stages` path. Only a green live run remains. | 🚫 **out of scope by design** — `main.rs:11833` hard-errors "blind_exit role is supported on Linux/macOS only"; like mobile clients, not a parity gap (corrected 2026-06-24, was ❌) |
