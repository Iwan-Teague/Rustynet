...
| anchor (gossip/bundle-pull/enrollment/port-map) | ✅ | 🟡 **bundle-pull**: implemented end-to-end (launchd `com.rustynet.anchor.plist` loopback bundle-pull profile + `ops install-macos-anchor` verify-before-serve installer + `macos_service_hardening` reviewed-shape validator + `ops e2e-bootstrap-macos` seeds the bundle-pull token + live `live_macos_anchor_test` bin asserting loopback byte-for-byte / token gate / LAN-bind refused / secrets hygiene, wired into the `validate_macos_anchor_bundle_pull` vm_lab stage, FAIL-LOUD: the live result is authoritative and the dry-run plan is informational-only, never a Pass) — **deploy machinery landed 2026-06-22** (`deploy_macos_anchor_profile` stage: standalone `ops seed-macos-anchor-token`, `amend_membership_for_macos` grants `anchor.bundle_pull` when elected, bootout client daemon → `install-macos-anchor` verify-before-serve → poll listener). **BLOCKED on a passphrase-config mismatch** (run `livelab-1782135034`, diagnosed on .210): the anchor daemon installs + kickstarts (exit 0) but crash-loops `config_error 65: wg key decrypt failed — load macOS keychain passphrase failed: os secure store unavailable`, so the listener never binds. Root cause: the reviewed `com.rustynet.anchor.plist` hardcodes keychain account `wg-passphrase-daemon-local` + passphrase file `/usr/local/var/rustynet/keys/wireguard.passphrase`, but the bootstrap actually provisions the **node-specific** account `wg-passphrase-macos-client-1` + file `/usr/local/var/rustynet/bootstrap/wireguard.passphrase` (the `/keys/` file does not exist). The anchor profile must reuse the **same** node-specific passphrase custody the client daemon (`com.rustynet.daemon.plist`) uses. FIX (security-sensitive — touches key custody + the verify-before-serve reviewed shape): template the anchor plist's passphrase account/credential-path from the live bootstrap config (or derive the anchor plist from the installed client plist), and update `macos_service_hardening.rs` to accept the templated values while still enforcing the loopback/allow-lan posture; gossip/enrollment/port-map still via the shared `live_linux_anchor_test --platform macos` path | 🟠 dry-run plan contract only (`validate_windows_anchor_bundle_pull_plan_contract`) |
| exit (NAT egress + route advertise + killswitch) | ✅ (nftables) | 🟡 implemented (`is_macos_active_exit`, `pf` killswitch precedence, exit-evidence capture) — needs a live green run | 🟡 implemented (`promote_windows_exit_active`, `validate_windows_exit_nat_lifecycle`) but 🔒 the lab Windows guest lacks the WinNAT/HNS stack (`MSFT_NetNat` absent) — see `WindowsExitNodeRunbook_2026-06-04.md` |
| blind_exit (irreversible exit) | ✅ | ❌ deferred | ❌ untested |
| relay (live session forwarding) | ✅ | 🟠 lifecycle dry-run only | 🟠 SCM lifecycle **contract** only (`validate_windows_relay_service_lifecycle_contract`, "without guest mutation") — no live forwarding |
