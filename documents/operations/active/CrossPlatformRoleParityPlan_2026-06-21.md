...
2. **Upgrade remaining contract/dry-run validators to live** — Windows relay
   ...
   live-proven; macOS anchor port_mapping_authoritative is live-proven
   2026-07-04 (`validate_macos_anchor_port_mapping_authority`); remaining
   anchor sub-surfaces (gossip_seed, enrollment_endpoint, and
   port_mapping_authoritative on Windows) still need explicit live coverage.
...
- **anchor (Windows/macOS):** bundle-pull serving is live-proven on macOS and Windows (`validate_macos_anchor_bundle_pull`, `validate_windows_anchor_bundle_pull`); `port_mapping_authoritative` is live-proven on macOS 2026-07-04 (`validate_macos_anchor_port_mapping_authority`); next coverage must prove gossip_seed / enrollment_endpoint live (both OS) and port_mapping_authoritative on Windows.
...
- **Cross-OS live role transitions:** macOS `LocalOnly` (admin<->client)
  ...
  `LocalOnly` (`windows_service` reload) also LIVE-PROVEN 2026-07-04
  (`validate_windows_role_transition`, `livelab-1783174602-844175f5ad2a`).
  Only the `SignedMembership` kind remains design-only for both OS — design at
  `CrossOsRoleSwitchPlan_2026-06-24.md` (note: Windows `StateRefresh` IPC
  already exists; reuse the single verified apply path
...
- **Live cross-OS role transitions — genuinely unbuilt (Linux-authorable).**
  ...
  **UPDATE 2026-07-04: both the macOS and Windows `LocalOnly` slices of this
  are now built AND live-proven** (`validate_macos_role_transition`, run
  `livelab-1783135864-2fda3979d599`; `validate_windows_role_transition`, run
  `livelab-1783174602-844175f5ad2a`, commit `5516711`, after the Windows CLI
  IPC-client gap was closed by `c51f00a`) — see §3 above. Only the
  `SignedMembership` kind remains unbuilt for both OS.

**Net remaining parity CODE work (no live lab):** (a) the
`SignedMembership` transition kind for both OS; (b) the mac/win
stage-gating contract tests (Linux-authorable, small); (c) explicit live
stages for the remaining anchor sub-surfaces beyond bundle-pull and
port_mapping_authoritative (macOS `port_mapping_authoritative` landed
2026-07-04): gossip_seed and enrollment_endpoint on both OS, and
port_mapping_authoritative on Windows (needs a `windows_membership_
capabilities` equivalent — no such function exists yet, Windows amend
uses inline capability strings). `enrollment_endpoint` specifically has
zero runtime enforcement today (the capability string is defined but
never checked) — that gap needs a design+implementation pass before any
live-lab stage can exercise it. Everything else is code-complete-
live-pending or HP-3/WinNAT-blocked.
