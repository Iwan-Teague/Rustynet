...
**Net remaining parity CODE work (no live lab):** (a) the Windows half of the
role-transition stage + the `SignedMembership` transition kind
(Linux-authorable, medium — macOS `LocalOnly` landed 2026-07-04); (b) the
mac/win stage-gating contract tests (Linux-authorable, small); (c) explicit
live stages for the remaining anchor sub-surfaces beyond bundle-pull
