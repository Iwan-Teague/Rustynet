...
| Port-mapping (uPnP / NAT-PMP / PCP) | `crates/rustynetd/src/port_mapper.rs` (D2.3) + daemon lease lifecycle in `crates/rustynetd/src/daemon.rs` (`maybe_refresh_port_mapping`) | Production. The protocol clients (D2.3) plus the daemon-side lease lifecycle now land: the reconcile loop re-runs the supervisor bring-up at ~half the lease TTL and immediately on a local endpoint change, re-checking the elected authority each cycle. Re-running bring-up (vs `refresh_existing_lease`) re-resolves the internal client address, so a DHCP-reassigned internal IP gets a fresh mapping automatically. Multi-anchor coordination is Pin-then-Seniority (FIS-0014): pinned node first (`anchor.port_mapping_pinned`), then oldest `joined_at_unix`, then node id. |
...
Pass criterion: integration test — anchor A is up; new device N has the membership owner public key but no bundle; N runs `rustynet anchor pull-bundle` with a valid token; N obtains the bundle, verifies it against the public key, and joins gossip. **The "verifies it against the public key" step is implemented**: `anchor pull-bundle` accepts a bundle only when its membership head attestation verifies against the pinned §6.B owner key (owner signature + quorum + freshness + epoch monotonicity) via `rustynet_control::membership::verify_attested_snapshot`, before any byte is written — see SecurityMinimumBar §3 control 2 and the `pull_bundle_never_writes_unverified_bytes` / `anchor_bundle_pull_serves_attested_snapshot_that_client_verifies` tests.
...
| Bundle-pull payload is cryptographically authenticated (membership head attestation) | `verify_attested_snapshot` in `crates/rustynet-control/src/membership.rs`: `anchor pull-bundle` accepts a snapshot only when its head attestation binds the recomputed state identity to the §6.B pinned owner key (pinned-owner signature + attested-quorum + bounded freshness + epoch monotonicity/fork detection), before any disk write; anchors materialize but never mint attestations | Unit tests `verify_attested_snapshot_rejects_*` (incl. the roster-lists-pinned-key-without-its-signature case) in `membership.rs`; integration tests `pull_bundle_never_writes_unverified_bytes` (rustynet-cli) and `anchor_bundle_pull_serves_attested_snapshot_that_client_verifies` (rustynetd) |
...
| Multi-anchor port-mapping coordination | Anchors with `port_mapping_authoritative=true` use signed precedence rather than racing the router — Pin-then-Seniority (FIS-0014): a node holding `anchor.port_mapping_pinned` wins, else oldest `joined_at_unix`, then node id. Operators set/clear the pin with `rustynet role pin-port-mapping-authority [--node <id> | --clear] --output <path>` (mints a `SetNodeCapabilities` proposal; normal sign/apply quorum path; refuses ineligible targets, warns on multi-pin and on mixed-version fleets — the new capability token fail-closes bundle decode on pre-FIS-0014 daemons, so pin only after full fleet upgrade) | Integration test: two anchors on the same simulated LAN — only the elected authority requests the lease |
...
- keeps any uPnP/NAT-PMP port-forward lease valid (the lease points at an internal IP; if that IP moves, inbound WAN traffic breaks until the mapping is re-issued for the new internal IP). The daemon now re-leases automatically on a detected endpoint change (`maybe_refresh_port_mapping` in `crates/rustynetd/src/daemon.rs`), so the break is bounded to a reconcile tick + gateway round-trip rather than a full lease TTL — but a DHCP reservation removes the break entirely and is still the recommended belt-and-braces,
