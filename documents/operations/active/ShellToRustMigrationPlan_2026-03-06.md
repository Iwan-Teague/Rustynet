...
- Phase G progress: cross-network evidence validators/generators now execute through Rust ops only (`generate-cross-network-remote-exit-report`, `validate-cross-network-remote-exit-reports`, `validate-cross-network-nat-matrix`, `validate-network-discovery-bundle`); legacy Python entrypoints are compatibility wrappers.
- Phase G progress: fresh-install OS matrix input rebinding/report generation and live-lab failure digest generation now execute through Rust ops only (`rebind-linux-fresh-install-os-matrix-inputs`, `generate-linux-fresh-install-os-matrix-report`, `generate-live-linux-lab-failure-digest`), and the orchestrator canonicalized-input handoff no longer shells through inline Python JSON parsing.
- Phase G progress: `live_linux_cross_network_direct_remote_exit_test.sh` now uses the same Rust cross-network report generation path as other cross-network validators (single hardened schema/trust path).
- Phase G progress: active cross-network E2E scripts no longer rely on inline Python for report parsing/topology classification/roam alias selection/monitor summary generation; these flows now dispatch to Rust ops (`read-cross-network-report-fields`, `classify-cross-network-topology`, `choose-cross-network-roam-alias`, `validate-ipv4-address`, `write-cross-network-soak-monitor-summary`) for one hardened parser path.
- Phase G progress: `live_linux_lab_orchestrator.sh` inline Python helpers are removed from active execution paths; topology checks, secure local file-mode validation, forensics redaction/manifest generation, discovery digesting, cross-network preflight report writing, reboot-recovery report generation, run summary generation, and outage SSH scan diagnostics now dispatch through dedicated Rust ops commands for one hardened parser/evidence path.
