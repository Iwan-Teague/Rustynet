...
- [SecurityReview_2026-05-24.md](./SecurityReview_2026-05-24.md) — firm-grade security review across six domains (privileged boundary, crypto/key custody, trust/anti-replay, untrusted-input/IPC, dataplane fail-closed/killswitch, secret-hygiene/supply-chain). 38 findings (7 High / 9 Medium / 17 Low / 5 Info) with CWE, file:line, exploit scenario, and a prioritized P0/P1/P2 remediation roadmap; load-bearing findings verified first-hand. Headline P0s: membership-decoder DoS (unbounded `Vec::with_capacity`), dataplane fail-open paths (dead `dataplane.rs`, discarded `force_fail_closed`, bootstrap leak window), Windows killswitch/IPv6 leaks, and policy default-allow (revocation bypass)
