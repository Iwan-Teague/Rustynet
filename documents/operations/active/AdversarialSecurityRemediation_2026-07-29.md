...
| PF-05 | Killswitch assertions check presence, not precedence | **See S2** | M | **DONE 2026-09-09** — `8417edf1` built the macOS precedence evaluator but never rewired the live `MacosCommandSystem::assert_killswitch` (`phase10.rs`), which stayed a substring check until the 2026-09-08 ledger audit (`NodeEngineAuditConsolidation_2026-09-08.md` §2-D) caught the tick; the live assertion now walks the anchor through `evaluate_macos_killswitch_rules_acknowledging`, passing the daemon's own egress interface as ACKNOWLEDGED when it set `allow_egress_interface` — the exact mirror of the Linux S2 handling — so the PF-01 egress-wide pass is reported rather than silently credited and every other `quick` pass above the terminator fails; `macos_assert_killswitch_rejects_a_quick_pass_above_the_terminator` is the negative test and the QH-29 agreement test pins that the exit posture FAILS the strict walk (PF-01 detected) and passes only with the acknowledgment. Linux (`c047358f`) and Windows (`d40323e8`) were wired in S2. |
