...
- Step 2 — `vm_lab/mod.rs`: `exercise_macos_reboot_recovery_live` renamed to `exercise_macos_reboot_recovery_with_recovery_actions` with a `post_daemon_live(&str) -> Result<(), String>` seam invoked exactly once after the daemon-live probe (carrying the parseable node id) and before the pin/marker/boottime probe; a hook error fails the whole exercise (fail closed). All pre-existing checks (boottime change, daemon-live, recovery line via root-run grep, residue marker absent, typed failclosed check) unchanged.
- Step 3 — stage `macos_reboot_recovery_validation.rs`: installs the seam; redistributes FRESH `Traversal` then `DnsZone` bundles via the new `distribute_bundle_kind_scoped` (same issue + verifier-key barrier + signed-bundle install as setup, scoped to the rebooted node), then polls `rustynet status` through the adapter's `shell_host` (`RUSTYNET_DAEMON_SOCKET=/private/var/run/rustynet/rustynetd.sock`) up to 180 s at 10 s until `state` is one of the allowlisted healthy terminal states (`DataplaneApplied` | `ExitActive`, review F1 — a missing state or any other token never recovers) AND `path_programmed_peer_count >= 1`. Exhaustion/requery errors fail with the exact last status line; never a skip. `plan.rs` passes the run's worker/flag pair to the stage (now `::new`-constructed).
- Step 4 — tests (stub adapters + fake shell): one verifier-key + one signed-bundle distribute per kind on the rebooted node with ZERO distributions to other nodes; FailClosed-stuck poll fails carrying the verbatim status line; poll converges only on `state != FailClosed && path_programmed_peer_count >= 1` (missing count parses fail-closed to 0); redistribution-error fails closed naming the kind; source test pins `post_daemon_live(&node_id)` before the post-reboot probe construction and `distribute_bundle_kind_scoped` before `poll_generation_recovery`. Gates: `cargo fmt --all -- --check` PASS; `cargo clippy -p rustynet-cli --all-targets --all-features --features vm-lab -- -D warnings` PASS (0 errors); `cargo test -p rustynet-cli --all-targets --all-features --features vm-lab` PASS (reboot-recovery filter 23/23 first, then the full crate — no failures).

## STATUS 2026-09-07T13:05Z

- Follow-up (a) review remediation — DONE (edit job `edit-1788781850025-90987-0`, branch
  `ai-edit/edit-1788781850025-90987-0`; commits `1290f68f` + docs): the adversarial review
  (`MacosRebootCellRefreshReview_2026-09-07.md`, committed verbatim as `05cd36a9`) findings F1–F6
  are fixed. F1 is the substantive one: the recovery poll now ALLOWLISTS the healthy terminal
  `DataplaneState` tokens (`DataplaneApplied` | `ExitActive`, source-pinned to
  `crates/rustynetd/src/phase10.rs`) instead of negating `state != FailClosed`, so a missing state
  field or a drifted fail-closed token family can never read as recovered. Full dispositions in
  the review doc's `## Disposition (edit job edit-1788781850025-90987-0)` section.

- Follow-up (a) — DONE in the edit worktree (branch `ai-edit/edit-1788778485008-2108-0`; commits cc5735b1, 29776167, 1e9383d0): the macOS reboot-recovery cell redistributes FRESH signed traversal + dns_zone bundles to the rebooted node after the daemon-live probe and before the pin probe, then waits (≤180 s, 10 s poll) for the daemon to leave FailClosed with a programmed generation; redistribution errors / stuck FailClosed / absent pins all fail the stage with the exact status text. fmt + clippy (-D warnings) + full rustynet-cli test suite pass.
- Live proof: NOT done here — reboot attempt 4 remains owner-scheduled (no lab runs, no SSH, no VM control from this job). Expected pass shape: pre-reboot evidence pass → reboot → daemon-live → `distribute_bundle_kind_scoped` Traversal + DnsZone on the rebooted alias → poll exits on a programmed generation → pin/marker/boottime probe green → stage PASS.
- Next (owner): run reboot attempt 4 on `main` after merge; verify the appended `live_lab_node_run_matrix.csv` row and the stage's own report artifact (never the column alone).
