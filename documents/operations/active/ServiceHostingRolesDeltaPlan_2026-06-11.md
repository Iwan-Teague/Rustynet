...
- **Open (c/d):** test batches (store/protocol/session/enforce/bin gates incl. fuzz); daemon runtime integration (exposure controller + access-dir materialisation from signed policy); MagicDNS names; live-lab evidence rows; `nas_default_deny_gates.sh` / `llm_default_deny_gates.sh` / `llm_exit_coexistence_gates.sh` (land with the test batches).
