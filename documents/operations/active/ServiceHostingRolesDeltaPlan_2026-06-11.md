...
- **Status: 🔄 code landed 2026-06-11 (`493a994`); tests batched, daemon runtime wiring open.** Landed: `TrafficContext::{NasService,LlmService}` + the **empty-context hardening** (a rule with an empty `contexts` list no longer matches service contexts — a pre-D13 wildcard-context rule must never silently grant NAS/LLM access; service access requires naming the context explicitly); `LlmAccessScope`/`LlmScopePolicy` (restrictions on a grant, never a grant source; `None` scope = unrestricted grant); `rustynetd::service_exposure` (E1 bind validation incl. loopback-only engine check, `VerifiedPeerIdentity` handoff, `evaluate_service_access` seam, `ServiceHostingView` from signed membership, deterministic `ServiceExposureController` with `capability_release_ready` as the E3 enforcement point, redaction-safe `ServiceAccessEvent`); nftables `render_service_port_tunnel_scope_table`; **exit-coexistence guard** `enforce_overlay_exception_for_exit_routes` wired into both daemon apply paths (refuses an `ExitNodeDefault` route set without mesh routes — fail-closed, LLM design §6). **Closed since:** the truth-table/controller/guard test batches + five gate scripts landed (`70641c9`); the daemon now derives and atomically materialises `grants.v1`/`peers.v1`/`scopes.v1` from verified signed state at all four signed-state commit points, and clears the access dirs when the capability is absent (`de02cc2`, `service_access_state.rs`, 8 tests). Revocation severance is enforced by the binaries' per-frame re-read of the materialised deny + the orchestrator-side `ServiceExposureController` E3 gate.

### D13.c/D13.d implementation status (2026-06-11, commits `493a994` + `b2aad02`)
- **Landed (c):** `rustynet-nas` crate — AEAD-at-rest store (XChaCha20-Poly1305 via new `rustynet_crypto::aead_seal/aead_open`, AAD binds every blob to `nas:<kind>:<peer>:<name>` so cross-namespace replay fails the tag), per-peer namespaces + sealed quota records, keycheck sentinel, owner-only data-root checks (non-Unix refuses fail-closed), length-bounded deny-on-malformed codec, health probe; daemon binary (tunnel-shaped bind only, key via `LoadCredentialEncrypted`, per-frame grant re-check, deny-all without signed access state); systemd unit + generalised installer + Linux executor dispatch.
- **Landed (d):** `rustynet-llm-gateway` crate — node-signed short-TTL single-audience session tokens re-checked against current policy on every use (E4), model/quota/rate enforcement, loopback-only engine boundary (`InferenceEngine` trait + `MockEngine` — the doc-scoped stand-in), length-bounded streaming protocol; daemon binary with per-frame AND per-token-event grant re-check (revoke severs mid-stream); `rustynet llm allow|deny|access list` (unsigned-record-for-owner-signing; read-only list reports default-deny honestly); systemd unit + installer + executor dispatch; exit-coexistence guard (see D13.b status). **Transport resolution:** the gateway speaks a hardened framed binary streaming protocol delivering the gRPC/HTTP-2 properties the design names (persistent connection, binary frames, streamed tokens, no handshake/TLS overhead) with zero new audited dependencies; the §5.4 contract is transport-agnostic and a gRPC/SSE/QUIC adapter can layer on later.
- **Open (c/d), updated after `70641c9` + `de02cc2`:** test batches ✅ (store/protocol/session/enforce incl. deterministic no-panic fuzz loops); gate scripts ✅ (`service_hosting_role_gates.sh`, `nas_default_deny_gates.sh`, `llm_default_deny_gates.sh`, `llm_exit_coexistence_gates.sh`, `role_transition_audit_gates.sh`); daemon access-state materialisation ✅. Still open: MagicDNS overlay names for service hosts (`vault.nas.<mesh>` / `brain.llm.<mesh>` zone entries); Linux live-lab evidence rows (M5 — blocked this session on the in-flight vm-lab simulator stream; run after that lands); macOS/Windows stay ⛔ per the platform matrix.
