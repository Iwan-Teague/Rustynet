...
- 2026-09-07 (a) LAB gates: `cargo fmt --all -- --check` clean; `cargo clippy -p rustynet-cli -p rustynet-mcp --all-targets --all-features -D warnings` clean (pinned 1.88.0 toolchain); `cargo test -p rustynet-cli --all-targets --all-features --features vm-lab` exit 0 across all 93 test binaries (incl. plan count pins, registry extensibility/tier pins, run-matrix oracle equivalence, repo-context doc pin). Commits on `ai-edit/edit-1788775747880-75665-0`: 87f23584 (log), 1e29a9e5 (stage + wiring), e31333f5 (registry/oracle/doc), 4cf4e260 (TTL hard error).
