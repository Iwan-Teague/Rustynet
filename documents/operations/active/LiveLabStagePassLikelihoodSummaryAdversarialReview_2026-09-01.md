...
| §2 candidate 4 — `--enable-soak-suite` selector | ~~READY-WITH-AMENDMENTS~~ **WITHDRAWN** (**Corrected 2026-09-02** — premise refuted) | ~~Claim verified except the `EnableRule::SoakSuite` anchor is stale by a few lines (rule at `:347`, not `:336-346`) (§4).~~ Superseded: this review's own §4.3 cited "365 skip / 40 pass" in the per-node ledger — that 40 Linux pass is direct evidence `extended_soak` HAS dispatched and passed on `--node`, and the review should have concluded the stage is reachable (it dispatches in the default Linux full-suite plan; mac/win all-skip columns are the `--skip-linux-live-suite` fast path by design). The candidate is withdrawn; the anchor correction stands (§4 corrected note). |
...
> **Corrected 2026-09-02:** This candidate is **WITHDRAWN** — its READY-WITH-AMENDMENTS verdict is superseded and rests on a refuted premise. This review's own §4.3 cited "365 skip / 40 pass" in the per-node ledger; that 40 Linux `pass` is direct evidence `extended_soak` HAS dispatched and passed on `--node`, and the review should have concluded from it that the stage is reachable. It is: `plan.rs`'s `include()` has `StageSuite::Soak => !skip_live_suite && !skip_soak`, so soak is in the DEFAULT plan of every full-suite run (`--skip-soak` opts out); the mac/win all-skip columns are the `--skip-linux-live-suite` fast path by design. "Unreachable by any selector" (§4.3 below) is false — and an opt-in `--enable-soak-suite` flag would have REMOVED soak from the default plan, which is why the proposal was rejected. The stale-anchor correction in §4.1/§9.2 stands; everything below is retained as the retracted original.
