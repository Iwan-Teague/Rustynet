...
- RSA-0023 enrollment ledger → wrap the read-modify-write in an OS advisory lock (mirror `resilience.rs::acquire_lock`); add the §6 concurrent-consume race test. **M** — ✅ APPLIED 2026-06-24 (daemon path: acquire_ledger_lock flock spanning load→consume→write; 8-thread race test = exactly one redemption). ✅ EXTENDED 2026-06-26: the CLI operator `enrollment admit` path (`rustynet-cli` `execute_enrollment_admit`) was a SECOND, still-unlocked consume path the original fix missed — it did load→consume→write without the lock, so two concurrent CLI admits (or a CLI admit racing the daemon) could double-spend a single-use token. Now redeems via `consume_enrollment_token_locked` (same flock-guarded load→consume→write); 8-thread CLI race test asserts exactly-once.
...
- **Input/robustness:** RSA-0027 (structural CIDR parse, RN-N7 — ✅ APPLIED 2026-06-24), RSA-0050 (sysinfo parser bounds — ✅ APPLIED 2026-06-24), RSA-0033 (helper kill scoped to owned PIDs), RSA-0055 (CSV formula injection — ✅ APPLIED 2026-06-24), RSA-0054 (matrix report path confinement — ✅ APPLIED 2026-06-24), RSA-0066 (e2e host-key pinning), RSA-0041 (relay reject reflection), RSA-0056/0061 (orchestrator/e2e argv hygiene), RSA-0070/0071/0072 (script robustness/gate-drift/HB-3), RSA-0058 (printf quoting — ✅ APPLIED 2026-06-24). **M total**
