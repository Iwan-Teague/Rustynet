...
- 2026-05-27 to 2026-05-28 — Track A live evidence attempts (retries 38–43, run matrix). Defects found and fixed during the `live_anchor` stage: `a1c064f` (anchor sub-caps missing from genesis membership), `56b1776` (anchor bundle-pull token scope wrong for admin nodes), `27b0e39` (macOS default route lost after lab restart — macOS preflight now auto-detects and restores), `1c254ff` (env_logger not initialized in rustynetd → anchor events silent in journald, causing `validate_bundle_pull_log_redaction` false-negative), `e1652c1` (orchestrator `--skip-to <stage>`, auto matrix-row finalization on EXIT trap, journalctl flush retry 3-attempt backoff). Track A sub-stages A1–A3 live pass criterion remains open pending a clean run against current HEAD.
