...
### QH-64 — A mid-run rustynetd restart can hit a missing-trust-evidence race that sticks the node in `RestrictionMode::Permanent` for the rest of the run, and gossip acceptance goes silently to zero
**Severity: high (fail-closed trust/security state; the observable symptom looks like a network bug and wastes lab cycles chasing it). Confidence: VERIFIED — live daemon journal from a real failed run, 2026-08-30.**

Live-lab run `labrun-1788118517051-33199-0` (Linux-only, `debian-headless-2` + `lenovo-exit-1`) failed at `gossip_convergence_validation`: `lenovo-exit-1: gossip_accepted_total=0 (registered peers, but never verified a bundle from one) (after 150s)`. The obvious hypotheses — a gossip-protocol bug, a role-specific nft firewall rule on the exit node, a broken WireGuard tunnel — were each checked and each **ruled out** by direct evidence, in order:

1. **Not a gossip-protocol bug.** `debian-headless-2`'s own journal shows it successfully accepting gossip minted by `lenovo-exit-1` the whole time (`gossip_accept source=4b9d05b552c8b56e seq=N candidates=2`, repeated) — the failure is asymmetric, not a two-way outage. Reading `crates/rustynetd/src/gossip_runtime.rs`'s accept path (the `log::info!("gossip_accept …")` at `:710-715`, inside `validate_and_apply_inbound_bundle`) shows no role-conditional branch — the checks before it (membership epoch present, per-origin rate budget, watermark persist) are generic, not exit-vs-client differentiated.
2. **Not a firewall rule.** `sudo nft list ruleset` on `lenovo-exit-1` was completely **empty** at the time of inspection — no table, no chain, nothing to block inbound gossip on port 51821/UDP (`crates/rustynetd/src/gossip_transport.rs:60`, `RUSTYNET_GOSSIP_PORT = 51821`).
3. **Not (directly) a WireGuard tunnel failure**, though this one is a genuine trap: `wg show` and `ip addr show rustynet0` both report the interface absent on the guest — but that reading was taken **after** the run's `cleanup` stage had already stopped the daemon and torn the interface down (confirmed: `systemctl is-active rustynetd` → `inactive`, no `rustynetd` process running, `disconnect cleanup complete` lines in the journal). Reading post-cleanup absence as evidence about the failure-time state would have been the exact QH-06/QH-10-shaped mistake this doc already warns about — the fix was to re-read the journal for the actual failure window instead of trusting current live state.

**The real mechanism, from `lenovo-exit-1`'s journal for the failure window (2026-08-30 19:49-19:56, `journalctl -u rustynetd`):**
- The daemon started cleanly at 19:49:02 (`gossip transport bound; gossip data plane active`), ran ~2.5 minutes with no trust errors, then was restarted by systemd at 19:51:38-19:51:40 (two rapid restarts, the first killed by SIGTERM, `Failed with result 'signal'`) — the THIRD daemon instance (pid 185700) is the one the gossip failure was measured against.
- That instance immediately hit: `[WARN] rustynetd::daemon: rustynetd reconcile fail-closed: trust reconcile failed: trust evidence is missing`, repeated 5-6 times over ~5 seconds (19:51:19-19:51:24 — note this is *before* 185700's own startup timestamp of 19:51:40 in wall-clock terms per the raw log, i.e. these lines interleave with the second, crashed instance; the sequence needs re-confirming against exact PIDs before a fix lands, flagged below), escalating through `restrict_recoverable` to `restrict_permanent: reconcile failure threshold exceeded: 5` then `6`.
- `load_trust_evidence` (`crates/rustynetd/daemon.rs:13691-13699`) is a **local file existence check** — `if !path.exists() { return Err(TrustBootstrapError::Missing) }` — against whatever path `--trust-evidence ${RUSTYNET_TRUST_EVIDENCE}` names (the systemd unit's own env var). This is a bootstrap-ordering / provisioning-race shape: the orchestrator's own bootstrap step evidently didn't have this file in place (or in place *again*, after the restart) before the daemon's reconcile loop started polling for it.
- `RestrictionMode::Permanent` **does not self-heal** within a running process. Verified: `restrict_permanent` (`daemon.rs:10773-10777`) sets the mode; `restrict_recoverable` (`daemon.rs:10763-10766`) explicitly refuses to downgrade out of it (`"node already PERMANENTLY restricted; new failure while restricted"`); the only writes back to `RestrictionMode::None` found were at construction (`:5156`) and a handful of call sites (`:6015`, `:8843`, `:10557`) whose *gating* against an existing Permanent state was **not verified** in this pass — this is the one open question that would change the conclusion, see below. If none of those gates fire under Permanent, recovery requires nothing short of a daemon restart, and a node that's `restrict_permanent`'d almost certainly refuses to treat inbound gossip as trustworthy under this project's fail-closed posture (AGENTS.md §3) — which is consistent with, and very likely *is*, the actual mechanism behind the observed `gossip_accepted_total=0`.
- A secondary symptom in the same window: `rustynetd: flap breaker for peer debian-headless-2-bootstrap closed -> open (intensity 0.66)`, escalating to `0.94` — plausibly a downstream effect of the same restriction (repeated failed reconciles look like instability to the flap detector) rather than an independent cause, but not confirmed either way.

**What this is NOT (ruled out above) and what remains open:**
- Not a gossip protocol defect, not a role-specific firewall gap, not (directly) a broken tunnel.
- **Open:** why the trust evidence file was unavailable to the third daemon instance specifically — a genuine bootstrap-ordering race in the orchestrator (the restart happened mid-run; something may have removed or not yet restored the file before the daemon came back up), a lab-environment-specific fluke, or something else. This needs the orchestrator's own bootstrap/role-reassignment code path traced against the exact timestamps, which this pass did not complete.
- **Open:** whether any of `daemon.rs:6015/8843/10557` actually clears `Permanent` under the right conditions — if one does, "sticky by design" above is wrong and this may already partially self-heal, changing the fix from "add recovery" to "the recovery path itself has a bug."
- **Open:** whether `RestrictionMode::Permanent` blocking gossip acceptance is confirmed by reading the gossip accept path's own restriction check (not yet located/read in this pass) or is an inference from the fail-closed convention.

**Disposition: OPEN, not fixed.** This is trust/security state (AGENTS.md §4: "Enforce signed control/trust state validation before mutation," fail-closed rules) — per this document's own QH-60/blind_relay convention, any change here needs a full plan + adversarial review cycle before code, not a same-session patch. Do not paper over `restrict_permanent`'s stickiness with an auto-heal without first confirming *why* the evidence went missing — if a real deployment can hit the same race, weakening the stickiness without understanding the race could turn a fail-closed control into a fail-open one.

**Update 2026-08-30, isolation evidence — the race is (so far) lenovo-exit-1-specific, not systemic.** Reproduced the identical failure once more (`labrun-1788121411287-38660-1`, same error text against the same node pair — recorded as a second stub in the triage ledger, no new information), then re-ran the SAME client (`debian-headless-2`) against a DIFFERENT exit node (`ubuntu-utm-1`, UTM-managed, not the libvirt-backed `lenovo-exit-1`) in `labrun-1788122807631-38660-2`. Result: `gossip_convergence_validation`, `traffic_test_matrix`, `role_switch_matrix`, and `exit_handoff` **all passed** — the mesh converged and gossip was accepted symmetrically with no restriction-mode involvement. This is one data point, not a proof (`lenovo-exit-1` was not restarted mid-run in this comparison run, so it does not yet distinguish "restart-triggered race" from "this specific guest's environment/provisioning"), but it does narrow the open question from §"why the evidence went missing" to specifically **what's different about `lenovo-exit-1`'s bootstrap/restart path** rather than a general protocol or accept-path defect — reinforcing that this is not, e.g., a `gossip_accept`-path regression that would affect every exit node. Next step when picking this back up: reproduce a mid-run daemon restart against `ubuntu-utm-1` (or another non-libvirt guest) deliberately, to test whether the race follows "any guest that gets restarted mid-run" or stays specific to `lenovo-exit-1`.
...
### QH-81 — the MeshStatus validator cannot express a peer expectation at all: the daemon reports advertised route CIDRs in a field named `expected_peer_ids`/`peer_ids`, so the peer-visibility check is structurally vacuous on every platform
**Severity: medium (a check that reads as proving peer visibility proves nothing; the live-handshake poll is currently the only thing making the stage meaningful). Confidence: VERIFIED — review
`MeshStatusEvidenceReview_2026-09-08.md` (this directory) against the live record
`{"op":"MeshStatus","passed":true,"report":{"peer_ids":[],"expected_peer_ids":[],"overall_ok":true}}`
and the code cited below.**

The daemon persists `SessionStateSnapshot.peer_ids` from `self.advertised_routes`
(`rustynetd/src/daemon.rs:9762-9765`) — advertised route CIDRs, not peer node ids —
so no node id can ever match a `peer_ids` entry, and the orchestrator-side
`probe_expectations` never emits `--expected-peer-id`
(`crates/rustynet-cli/src/vm_lab/orchestrator/stage/validate_runtime.rs`,
`probe_expectations` doc: emitting `ctx.node_ids` would red every node). The
result: `expected_peer_ids` is `[]` on every MeshStatus record, on every
platform, on every run shape — the field promises peer ids and cannot carry
them. A branch attempt to reject the empty expectation orchestrator-side
(commit `d944c700`) was REVERTED (2026-09-08): the rejection was
unsatisfiable by construction (no code path could make a ≥2-node run green),
so it would have failed `validate_baseline_runtime` on every multi-node run
and cascaded into `deploy_relay`/`traffic_test_matrix`/
`security_audit_validation`. The data model is mismatched, not the
orchestrator — relaxing the condition to rescue it was rejected for the same
reason.

The same gap extends to the legacy consumers:
`evaluate_linux_mesh_status_report` (`crates/rustynet-cli/src/vm_lab/mod.rs:23631-23656`)
accepts `overall_ok: true` with an empty `expected_peer_ids` (pinned by its own
tests, `mod.rs:50377-50401`), and the macOS/Windows focused-runner equivalents
never check the field. Both now carry an explicit vacuous-pass disclosure
pointing here.

**NOT orchestrator work.** This needs a daemon-side change: either stop
naming route CIDRs `peer_ids`/`expected_peer_ids`, or populate a real
expected-peer list the validator can assert against. Until then, LIVE peer
visibility is proven by the `mesh_status_validation` stage's live-handshake
poll (`path_live_peer_count` / `path_programmed_peer_count` /
`path_latest_live_handshake_unix`), and `validate_baseline_runtime`'s
MeshStatus op must be read as schema/staleness posture only.

**Disposition: OPEN, filed 2026-09-08 (daemon-side fix required).**
...
**Disposition: FIXED 2026-09-08.** Root cause is the deadline, not the spawn machinery: the probe budget (30 s, floored to 20 s by `local_process_probe_timeout`) is the only load-sensitive part of the test — under a loaded host the fork/exec of the stub `ps` (including Gatekeeper exec validation of a freshly written script, cf. AGENTS.md §7) alone can outlive the budget, turning scheduling delay into a false probe failure. Fix (worktree branch `ai-edit/edit-1788823521793-19065-0`): the test now injects `Duration::MAX` as the budget, which `run_output_with_timeout` can never take its timeout branch on, so the test's outcome no longer depends on wall-clock at all — deliberately not a 30→60 s widening. Regression coverage is unchanged and fails on behaviour, not on the clock: a narrowed argument list trips the `axww -o command` args-log assertion, and a broken needle match or UTF-8 handling trips the `present` assertion via the real spawn+parse path (`local_utm_process_present_with_ps` → `run_output_with_timeout` → `local_utm_process_present_in_ps_output`). Same wall-clock pattern NOTED nearby, not fixed (same flake class, stub spawns through the same `run_output_with_timeout` with `Duration::from_secs(30)`): `transition_local_utm_vm_skips_stop_when_vm_is_already_stopped` (budget at mod.rs:44730), `transition_local_utm_vm_skips_stop_when_utmctl_probe_spawn_fails_but_ps_reports_stopped` (:44776, two stub spawns = double the exposure), and `transition_local_utm_vm_accepts_timeout_when_vm_reaches_stopped_state` (:44838 — likely intentional, it exercises a wait-with-timeout path, so confirm semantics before changing).
...
| QH-89 | `negative_control_enrollment_token_replay` | NegativeControl | High | enrol once via the live anchor listener, replay the token sequentially and concurrently | both replays `already_consumed`, ledger has one consumed row, mesh peer count unchanged (orchestrator's own pubkey collection) | implemented (stage/`negative_control_enrollment_replay.rs`; gates green, adversarial GLM review addressed — verified early-path teardown; live verify pending lenovo-bot+katana) |
| QH-90 | `negative_control_killswitch_bypass` | NegativeControl | High | plant `pass out quick all` above the terminator from outside the daemon (Linux nft / macOS pf) | the live assertion FAILS naming the precedence error while planted, passes after removal; acknowledged-egress accept fires only when the daemon set the flag | SALVAGED, not merged — impl on branch `ai-edit/edit-1789134839761-87820-0` commit 461d434c (1768-line negative_control.rs addition; ATK1 job timed out). Branch was based on PRE-QH-89 main, so resuming needs a rebase onto current main + conflict resolution in negative_control.rs/mod.rs/plan.rs against the merged b81685eb (QH-89), then gate + adversarial review + merge |
...
| QH-96 | Setup/Live bin-report seam (12 rows) | Live | High | 12 bin-spawner stages gate on exit-code + non-empty report and never parse the report's rows, so "exit 0 with a report whose rows all say failed" passes (`live_two_hop_validation.rs:13-23,245`) | a shared host-side `passed_with_report` evaluator (pattern: `live_hello_limiter_flood_validation.rs:77`) asserts the per-row `ok` flags + measured datums, not just non-empty; affects live_anchor, two_hop, managed_dns, network_flap, reboot_recovery x2, enrollment_restart, lan_toggle, mixed_topology, secrets_not_in_logs, live_key_custody, macos_bundle_pull | IMPLEMENTED, not merged — branch `ai-edit/edit-1789146692816-49016-0` (tip 32c40bce, 12 commits, +2264/16 files): bin_report::passed_with_report helper + per-row verdict enforcement across all 12 stages; macOS reboot-recovery evidence JSON honestly marked unparseable (left on old check w/ TODO). Job hit overall timeout AFTER completing scope. Needs: full-workspace gate + adversarial review + merge. HELD during wind-down (audit I1) |
| QH-97 | Posture/self-inspection host cross-check | Live | High | runtime-acls, dns-failclosed, service-hardening, key-custody, security-audit, gossip, validate_baseline_runtime accept ONLY daemon self-reports — no independent observation (ToughPolicy rule 2) | after the daemon report passes, the stage itself runs `nft list ruleset` / `stat` / `systemctl cat` over RemoteShellHost (seam: `role_validation/blind_exit.rs:144,169`) and asserts the specific rule/permission claimed; a self-inspection that disagrees with the kernel fails | open (audit I1) |
| QH-98 | `verify_ssh` nodes=0 pass + `admin_issue` self-trust | Setup | Should-fix | `verify_ssh` returns Passed with an empty `nodes=0` witness on empty assignments; `admin_issue` trusts `status role=admin` | `verify_ssh` returns `Skipped("no node assignments")` like `mesh_status_validation.rs:96-100`; `admin_issue` runs the live identity challenge (`role_validation/identity_challenge.rs`) | open (audit I1) |
| QH-99 | Dataplane witnesses for relay/mesh/role-switch (I1 fixes 3,4,6) | Live | Should-fix | relay_validation, mesh_status, role_switch_matrix/exit_handoff pass on config/self-report without a byte-moved proof | relay: `ss -uamp`/`/proc/net/udp` counter delta > 0 across one probe; mesh_status: cross-check peer count vs host-parsed `wg show latest-handshakes`; role_switch/handoff: require parsed latest-handshake ≤180s and rx-bytes > 0 | open (audit I1) |
