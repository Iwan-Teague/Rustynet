...
- [x] Reduced helper reruns were used only to isolate the first failing live stage
  Evidence:
  - managed DNS isolation artifact: `artifacts/live_lab/20260412T_phase4_local_gates_worktree/live_linux_managed_dns_retry_report.json`
  - managed DNS isolation log: `artifacts/live_lab/20260412T_phase4_local_gates_worktree/live_linux_managed_dns_retry.log`
  - exit handoff isolation artifact: `artifacts/live_lab/20260412T_phase4_local_gates_worktree/live_linux_exit_handoff_retry_report.json`
  - exit handoff isolation log: `artifacts/live_lab/20260412T_phase4_local_gates_worktree/live_linux_exit_handoff_retry.log`

- [x] Stricter five-node local-gates evidence is green through `live_two_hop` for the right reason
  Evidence:
  - role-switch report: `artifacts/live_lab/20260412T_phase4_local_gates_worktree/role_switch_matrix_report_12ebe96.json`
  - exit-handoff retry report: `artifacts/live_lab/20260412T_phase4_local_gates_worktree/live_linux_exit_handoff_retry_report.json`
  - two-hop report: `artifacts/live_lab/20260412T_phase4_local_gates_worktree/live_linux_two_hop_report.json`
  - LAN-toggle report: `artifacts/live_lab/20260412T_phase4_local_gates_worktree/live_linux_lan_toggle_report.json`
  - managed-DNS retry report: `artifacts/live_lab/20260412T_phase4_local_gates_worktree/live_linux_managed_dns_retry_report.json`
  Notes:
  - `artifacts/live_lab/20260412T_phase4_local_gates_worktree/live_linux_two_hop_report.json` records `second_client_route_via_rustynet0 = pass`.
  - `artifacts/live_lab/20260412T_phase4_local_gates_worktree/live_linux_exit_handoff_retry_report.json` records a pass with the final client route still measured on `rustynet0` after the exit switch.

- [ ] Fresh-install evidence regenerated for current HEAD
  Blocker:
  - current Phase 4 provenance is still `SOURCE_MODE="working-tree"` with a dirty workspace recorded in `artifacts/live_lab/20260412T_phase4_local_gates_worktree/state/git_status.txt`
  ...
  Required next step:
  - create a clean commit for the current fixes, rerun the relevant live evidence on that commit, then generate `artifacts/phase10/fresh_install_os_matrix_report.json`

- [ ] Canonical cross-network evidence regenerated for current HEAD
  Blocker:
  - the five UTM nodes in `documents/operations/active/vm_lab_inventory.json` are all on the same underlay `utm-shared-192.168.64.0/24`, so they cannot honestly satisfy the repo's cross-network proof contract by themselves
  ...
  - topology contract: `documents/operations/active/UTMVirtualMachineInventory_2026-03-31.md`
  Required next step:
  - add pinned `known_hosts` coverage for `debian-lan-11`, verify SSH reachability, then run the cross-network suites on a clean commit so the six canonical reports under `artifacts/phase10/` are current and truthful
