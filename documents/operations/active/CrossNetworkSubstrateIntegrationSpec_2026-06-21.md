...
- **CN-1** ✅ **DONE** (2026-08-27, `crates/rustynet-cli/src/vm_lab/orchestrator/stage/cross_network/substrate.rs`).
  Introduce the three traits + `NetLeafRunner{LocalCommand,RemoteShell}` + a `MockLeafRunner`;
  no behavior change (existing enum path still runs). Pure additive, unit-tested against the mock.
  **Three deliberate deviations from the §0.1 verbatim signatures:**
  (a) the provider trait is named `CrossNetworkSubstrateProvider`, because the `CrossNetworkSubstrate`
  *enum* survives as the `--cross-network-substrate` CLI selector, and its lifecycle methods are
  `setup`/`teardown` rather than `provision`.
  (b) `SubstrateHandle` is a **struct, not a trait**: with the seam at topology level a handle is plain
  provisioned state (overlay/underlay address maps + the created-link list), and the per-node
  `NetLeafRunner`s are passed in by the caller as an alias→runner map rather than owned by the handle,
  so `leaf()` has no single sensible answer. `endpoint()` (→ `ResolvedEndpoint`, which also names the
  overlay-vs-underlay *plane* so a fallback cannot be mistaken for a provisioned overlay address) and
  `supports()` (→ `NatProfileId`/`Support`) exist as specified; `apply_nat_profile()` + `NatModifiers`
  are deferred to CN-4.
  (c) `NetLeafRunner::in_netns` is a **provided** method, not a required one, so no backend can
  hand-roll a weaker `ip netns exec` prefix; the namespace name is allowlist-validated before it enters
  argv, closing the shell-construction hole §4 forbids.
  The §0.4 lifetime risk is handled as prescribed: the handle is owned by `OrchestrationContext` and
  drained by the `always_run` `CrossNetworkSubstrateTeardownStage` — there is no `Drop` impl.
- **CN-2** ✅ **DONE** (2026-08-27, `crates/rustynet-cli/src/vm_lab/orchestrator/stage/cross_network/netns.rs`).
  `NetnsSubstrateProvider` (setup/teardown/supports) + the classify/filter port + deletion of the scp'd
  shell. The two lifecycle stages already landed in CN-1; CN-2 widened their hardcoded
  `VxlanSubstrateProvider` into a `topology_level_seam()` / `provider_for_record()` dispatch.
  **Deviations and decisions, each deliberate:**
  (a) **netns does NOT own the topology-level seam.** Its endpoints are namespaces inside ONE guest, not
  lab node aliases, so it provisions no cross-LAN overlay and `SubstrateHandle::endpoint()` answers on the
  underlay plane for every alias — `ctx.endpoints` is untouched, which is what keeps the default
  (`--cross-network-substrate netns`) path byte-identical to before. `topology_level_seam()` returns
  `NoOverlay(reason)` for netns and slirp rather than leaving the dispatch looking unimplemented.
  (b) **the simulator is provisioned per NAT profile by the classification gate**, not once at topology
  level: both gates rebuild a clean topology per profile/scenario precisely so conntrack state cannot hide
  a cold-inbound result, so holding one across thirty unrelated stages would be wrong as well as wasteful.
  (c) **teardown is a `rnsim-*` prefix SWEEP**, not a replay of the recorded resources: an aborted earlier
  run leaves residue no handle ever saw. Enumeration failures fail CLOSED ("could not list" must never read
  as "there was none"), names parsed off the guest are re-validated before entering argv, every removal is
  attempted even after one fails, and failures are reported joined.
  (d) **background processes are named transient systemd units**, not shell `&` + PID kills — there is no
  `&` across an argv-only SSH boundary, and a lost PID is an orphaned STUN listener that would silently
  corrupt the next profile's verdict. `--collect` garbage-collects the unit on stop.
  (e) `SudoRunner` composes the `sudo -n` prefix UNDER `NetLeafRunner::in_netns`, so namespace execution
  keeps CN-1's allowlist validation of the namespace name instead of hand-rolling the `ip netns exec`
  prefix — which would have re-opened the hole §4 forbids.
  (f) `CreatedLink` became `CreatedResource { alias, kind: ResourceKind, name }`, because a namespace and
  an interface need different removal verbs.
  **`supports()` per §D5.1 profile:** `port_restricted_cone` ✅, `full_cone` ✅, `symmetric` ✅ (all three
  are one nftables router: `masquerade`, `masquerade` + a prerouting `dnat` of the WireGuard UDP range, and
  `masquerade random` respectively); `baseline_lan` ❌ `UnsupportedByDesign` (it is the no-NAT same-LAN case
  and needs no substrate — every netns site is defined by its NAT boundary, so claiming it would report a
  NAT'd topology as the un-NAT'd baseline); `double_nat_cgnat` ❌ `UnsupportedByDesign` (needs a two-router
  carrier chain; a site has exactly one router hop — the typed form of `netns_internet_sim.sh:189`'s
  `exit 2`).
  **Shell deleted:** `scripts/vm_lab/netns_nat_classify.sh` and `scripts/vm_lab/netns_nat_filter.sh`, and
  the orchestrator's scp of all three netns wrappers. `netns_internet_sim.sh` is annotated superseded and
  retained only as `netns_daemon_path.sh`'s dependency and `vm-lab-network-audit`'s transit/mesh collision
  target; it is on no active orchestrator path. *(Corrected 2026-08-28: `netns_daemon_path.sh` is
  deleted, so the `vm-lab-network-audit` target is now the sim's only consumer and the sole thing
  keeping it.)*
  **Known cost:** every leaf op is one SSH round trip where the shell ran the whole sequence in one remote
...
- **CN-3** 🟡 **PARTIAL** (2026-08-28, `work/cn3-scenario-port`). Port the 8 scenario validators to
  ...
  **Deviations and decisions, each deliberate:**
  ...
  scope. Collapsing them means lifting four `main()`s into library functions — a separate change with
  its own review surface. What did change: their *evidence* is read in-process via
  `execute_ops_read_cross_network_report_fields` rather than by shelling to `cargo run … ops
  read-cross-network-report-fields`, which, built without `vm-lab`, lacked the verb and failed the stage
  after its assertions had already passed.
  (d) **`live_lab_write_ssh_trust_summary` is reproduced, not dropped.** The shell dumped the host keys
  its own ad-hoc ssh setup had pinned; that setup is gone. The artifact could not be dropped either —
  the report validator lists it in `required_pass_source_artifacts`, so a report without it cannot pass
  — so it now records what is actually true of the run: the participating targets, and that keys were
  orchestrator-pinned rather than accepted on first use.
  (e) **Bundle bytes cross the argv seam as single-line base64 through a FIXED script.**
  `substrate::validate_argv` rejects control characters, so a multi-line signed bundle cannot be an argv
  element. The script text is a compile-time constant and the path/mode/payload arrive as `$1`/`$2`/`$3`
  — no value is ever substituted into a command string. Node ids are additionally held to an allowlist
  (they become staging filenames) and spec fields reject the `;`/`|` delimiters the shell concatenated
  unchecked, where a node id carrying one could forge an extra signed-assignment entry.
...
- **CN-5** ✅ **DONE by W5.7** (2026-08-27, closed retroactively 2026-08-27 — the row's own work was
  ...
  **Remaining tail — DELETED 2026-08-28 (TRACKC-FIX-1), this row's last open item.** The tail was
  explicitly OUT of CN-5's original scope (it is not the orchestrator's duplicate stage set, and no
  part of it lived in `live_linux_lab_orchestrator.sh`), and it is now gone:
  `scripts/vm_lab/netns_daemon_path.sh` (536 ln, bash + inline `python3`), the dead
  `cross_network_daemon_path` bash-dialect entry in `live_lab_stage_registry.rs`, and its paired
  negative-vocabulary reference in `live_lab_run_matrix.rs` (that assertion pinned the
  unknown-`cross_network_*` prefix fallback, which is still pinned — with names absent from both the
  registry and `StageId`, so it cannot pass for the wrong reason).
  `scripts/vm_lab/netns_internet_sim.sh` **stays**: its second consumer,
  `vm_lab/network_audit.rs` (`NETNS_SIM_SCRIPT_RELATIVE_PATH`), is live, so the sim's own deletion
  remains gated on that and only that. Its header note was corrected to say so.
...
- **Dead sections:** §4.2/§4.3/§7 target `scripts/e2e/live_linux_lab_orchestrator.sh`, which no longer exists. The integration seam is now `orchestrator/plan.rs` + `stage/mod.rs` + `live_lab_stage_registry.rs`. The registry's `cross_network_daemon_path` bash-dialect entry was dead weight and was deleted 2026-08-28 (TRACKC-FIX-1).
