...
|---|---|---|
| **client** | 🟡 **DNS release-blocker CLOSED 2026-09-03; baseline chain now green** (`livelab-1788433705-bf4b1b1187c8`, commit `bf4b1b1187c8`, clean): macos-utm-1 as a full-tunnel client (`build_bundle_env` assigns every non-exit node the run's exit → `exit_mode=FullTunnel` → `FullyProtected`) now passes `validate_baseline_runtime`, `dns_failclosed_validation`, `security_audit_validation`, `runtime_acls_validation`, and `mesh_status_validation` — the first time a macOS client run advances through the full baseline+DNS+security+ACL+mesh validator chain on `--node`. Two fixes closed the 16-tick `DnsFailclosed` blocker: the daemon-side check now enumerates the nested `com.apple/rustynet_g{N}` pf DNS-block floor it was blind to (`2c10f9d9`, `macos_dns_failclosed.rs` — top-level `pfctl -s Anchors` never saw the nested anchor the daemon installs+verifies, so the check false-failed while the floor was present), and `dns_failclosed_validation` now derives the expected posture from the run topology instead of a role-based scoped guess (`e36a2295`). Neither weakened a control. **Still NOT overall-green:** `traffic_test_matrix` fails — now root-caused (CP-1 below) as a host/lab-topology gap (the UTM fleet is split across two isolated vmnet nets with no cross-vmnet underlay path), NOT a rustynet defect; the fix needs a host change (deferred, owner). ★ **Update 2026-09-05 (run #5, `livelab-1788631948-3aedcfff9a75`, commit `3aedcfff`, clean, report `state/live-lab-macos-client-stun2-20260905-175914`, topology macos-utm-1:client / debian-headless-4:exit / debian-headless-2:client, `--lab-stun-servers 74.125.250.129:19302 --linux-backend linux-wireguard-userspace-shared`): the STUN collection pipeline is now proven live end to end — `collect_pubkeys` PASSES for the first time with the backend pin (every node gathers reflexive candidates; run #4's kernel-backend stall is gone), `key_custody_validation` passes, and the run advances through the full matrix. `traffic_test_matrix` still fails (all four cross-vmnet legs 100% loss both directions; mac default-deny INCONCLUSIVE, failing closed) — consistent with CP-1: no peer link carried traffic across the vmnet split, so the gathered srflx candidates were never exercised by a real flow. Next lever is daemon-side traversal work (consuming `SRRLX_SPEC`/srflx when building the mac↔Linux peer paths), not lab plumbing. Run launched from the main tree, so its run-matrix row landed in the main tree's ledger. | 🟡 **bootstrap + dataplane-apply PROVEN 2026-09-05** (`run-2026-09-05-windows-11`, commit `9d55ac7bafc8`, clean, 46 pass/0 fail): first Windows `--node` run with zero failures — `bootstrap_hosts`, `collect_pubkeys`, all four `distribute_*` stages, `enforce_baseline_runtime`, `validate_baseline_runtime` all pass on `windows-x86-1`. Two same-day fixes: `collect_pubkeys` now reads the daemon's own status instead of a racy `wireguard.pub` file read (`70e16a63`); `enforce_baseline_runtime`'s WFP tunnel-permit filter was resolving the interface to a LUID *before* `backend.start()` created it (error 87 on every cold bootstrap) — fixed by deferring it to a new post-backend-start step (`3f7fc8d3`). Run used `--skip-linux-live-suite`, so mesh-traffic reachability (the actual client-cell bar) is **not yet proven** — see CP-4 update above. |
| **admin** | 🟢 `macos_admin=pass` (`livelab-1784501586`, commit `537e1901`, clean) — run overall failed on `two_hop` | ⬛ bootstrap blocker |
| **relay** (lifecycle) | 🟢 `macos_stage_relay_service_lifecycle=pass` (`livelab-1784497253`, `11620a6`, clean) ★ **GREEN on the engine of record 2026-09-06** — run `livelab-1788653310-04c85129bb4f` (commit `04c85129`, clean): `deploy_relay_service` AND `relay_validation` PASS on the `--node` engine (macos-utm-1:relay topology), after two fixes: the staged-plist deploy (98664b0e + 6e3bb383, attempt 3 `livelab-1788650472` got deploy green) and the `relay_validation` readiness wait `wait_until_ready` (attempt 3 triage: snapshot raced the kickstart by ~3 s; fix live-verified by attempt 4, stage artifact `logs/relay_validation.log` pass 00:05:21Z) | ⬛ / 🟠 SCM contract only |
| **relay** (frame-forwarding) | 🟠 **provable on demand, opt-in — two launch attempts, stage still never run (attempt 1: HP-3 skip-trap; attempt 2: macOS deploy-adapter defect)** (HP-3 wiring landed 2026-09-01: `relay_forwards_frame_validation` stage, `--enable-relay-forwarding-validation`; disrupts the mesh — nft blocks + two peer daemon restarts, QH-64 — so it is OUT of the default plan; first live pass still pending). ★ **Attempt 2026-09-05** (`livelab-1788636568-2a7cbf6ac22e`, commit `2a7cbf6a`, clean, topology macos-utm-1:relay / debian-headless-4:exit / debian-headless-2:client): launched with `--enable-relay-forwarding-validation --skip-linux-live-suite` and the relay stage was silently never planned — by design, not a bug: the stage is `StageSuite::Disruptive` and the plan gate is `!skip_live_suite && enable_relay_forwarding_validation` (`plan.rs:372`, regression-tested by `skip_live_suite_drops_relay_forwarding_validation_too`), so `--skip-linux-live-suite` drops it exactly like chaos/negative-control. The recorded selectors confirm `relay_forwarding_validation:false` (`native.rs:743` ANDs the flag with `!skip_live_suite`). The run itself was a clean setup-only pass (16 pass/0 fail/3 skip, `overall=partial` fast-path semantics) and its ledger rows are carried on this branch. **Correct recipe: elect the relay cell WITHOUT `--skip-linux-live-suite`** — the full Live suite then runs (with QH-64-aware eyes on `gossip_accepted_total=0`/`RestrictionMode::Permanent` after the stage's mid-run daemon restarts). ★ **Attempt 2, 2026-09-05** (run `rust-1788644094`, commit `0a118d10`, clean, correct recipe — no `--skip-linux-live-suite`, `--skip-soak`, same relay topology): the relay stage WAS planned this time (recorded selectors `relay_forwarding_validation:true`) but the cell still did not exercise — `deploy_relay_service` FAILED on macos-utm-1 (`sh: line 0: cd: /Users/mac/Rustynet: No such file or directory`): the macOS deploy adapter ran `ops install-macos-relay` from a guest source checkout (`workdir`, else `$HOME/Rustynet`) that a `--node` bootstrap never materializes (lab-tooling defect, not a rustynet defect; the earlier relay-lifecycle green row predates the `--node` engine). `relay_validation` and `relay_forwards_frame_validation` skipped as failed-dependency. Second fail = `traffic_test_matrix` CP-1 cross-vmnet 100% loss (owner-deferred, unchanged). Tally 23 pass / 2 fail / 41 skip of 66 planned. NO ledger rows were appended — evidence finalization itself failed (transient_failure 70), so the row absence is recorded here, not fabricated. Fix in flight: upload the reviewed plist from the orchestrator workspace + temp staging cwd (no guest source root) ★ **Attempt 3, 2026-09-05/06** (run `livelab-1788650472-ef1c5ed0ec20`, source commit `6e3bb383`, clean, same relay topology, correct recipe): `deploy_relay_service` **PASS on macOS for the first time** — the staged-plist deploy fix (98664b0e + 6e3bb383) works. New blocker one stage later: `relay_validation` FAIL — startup race (snapshot captured in the same second as the deploy kickstart; relay reached full readiness ~3 s later; post-run guest state healthy: UDP 127.0.0.1:4500 + TCP 127.0.0.1:4501 LISTEN, `/healthz` ok). Remedy landed on the GLM manager branch: bounded `wait_until_ready` (30 s/1 s) in `role_validation/relay.rs`. `relay_forwards_frame_validation` skipped (failed dependency); `traffic_test_matrix` CP-1 fail as before (24/2/40). ★ **CP-1 elevated to structural blocker for this cell (2026-09-06)**: live probe from debian-headless-2 showed 192.168.65.101 unreachable at the LAN level (ICMP 100% loss + TCP/22 timeout), so no Linux peer can frame the macOS relay at all; the stage also fail-closes non-Linux relays by design (`relay_forwards_frame_validation.rs` platform gate; `select_relay_forward_test_topology` elects Linux only). A macOS probe port is moot until the owner resolves CP-1. Attempt 4 (readiness fix live-verified) recorded in `GlmManagerLog_2026-09-05.md` ★ **Attempt 4, 2026-09-06** (run `livelab-1788653310-04c85129bb4f`, commit `04c85129`, clean): `relay_validation` **PASS** (readiness fix live-verified; lifecycle cell closed above), and `relay_forwards_frame_validation` now has its durable fail-closed record in the ledger — it failed exactly at the platform gate: "relay node macos-utm-1 is Macos; the relay-frame-forwarding probe is Linux-only (nft + systemd unit restarts)". Combined with the LAN-level CP-1 evidence (ICMP + TCP/22 dead between vmnets), this cell is **blocked pending CP-1** — a macOS probe port is moot until peers can reach the macOS guest at all | 🔒 HP-3 (same opt-in stage; not yet elected on Windows) |
| **anchor** | 🟢 ★ **Re-proven 2026-09-07** (`livelab-1788756583-b4c3097999cd`, commit `b4c3097999cd`, clean, report `state/live-lab-macos-anchor-20260907-043630`, topology macos-utm-1:anchor / debian-headless-4:exit / debian-headless-2:client, `--skip-linux-live-suite --linux-backend linux-wireguard-userspace-shared --source-mode local-head`): 20 pass / 0 fail / 2 legit skips (admin_issue, blind_exit — not elected); all four anchor stages green again (`anchor_validation`, `deploy_macos_anchor_profile`, `validate_macos_anchor_bundle_pull`, `validate_macos_anchor_port_mapping_authority`); 0 evidence-finalization failures; matrix row appended (332nd, `macos_anchor=pass`). Launched from edit worktree edit-1788754429901-7876-0 with the target-pinned binary, so provenance recorded the binary's build worktree (28885-0) and the row landed in that worktree's ledger — owner reconciles at merge. **GREEN 2026-09-05** (`livelab-1788635706-1eb6e800c4cf`, commit `1eb6e800`, clean — all four macOS anchor stages pass on the `--node` engine, incl. `validate_macos_anchor_port_mapping_authority` against the daemon identity `macos-utm-1-bootstrap`; see end of cell for the QH-68 history and the finalization fix this run also carried). **MAC-D1 rerun CONFIRMED 2026-08-31** (`livelab-1788165016205-17194-3`, commit `5db953ad`, clean, 16 pass/0 fail/3 skip). `bootstrap_hosts` → `validate_baseline_runtime` all pass, and critically **all six baseline validators pass on macos-utm-1 including `DnsFailclosed`** (RuntimeAcls/ServiceHardening/KeyCustody/Authenticode/MeshStatus/DnsFailclosed all `passed:true` — the DnsFailclosed enforcement gap [[exit row below]] evidently only bites in the exit/protected-DNS-mode posture, not plain anchor). `anchor_validation` itself still `skip`s ("no node executed this validation; 1 node(s) reported a runtime skip") — **CORRECTED 2026-08-31, this is NOT a posture-gate issue** (the earlier attribution to `is_supported_for_platform` was wrong; that gate is deliberately never consulted by this stage, per MAC-D1's own de-circularisation). Root-caused in `AnchorValidationMacosPostureGateInvestigation_2026-08-31.md`: `--skip-linux-live-suite` drops the three macOS anchor validator stages (`deploy_macos_anchor_profile`, `validate_macos_anchor_bundle_pull`, `validate_macos_anchor_port_mapping_authority`, all tagged `@ Live`) out of the plan entirely, so MAC-D3's election-tightening check (`native.rs:376-383`) correctly falls back to `false` and the stage correctly fail-closed-skips — but this means the macOS anchor cell can **never** be exercised on the fast path, contradicting the documented `skip_linux_live_suite` contract ("the mac/win stages stay fully exercised"). Two-part fix landed: (a) the plan-builder is now anchor-election-aware so `--skip-linux-live-suite` retains the three stages when a macOS anchor is elected (commit `e3297391`), and (b) the election is now derived from the `--node` anchor assignment, not just the raw `--anchor-platform` flag, so the `ai_lab_run`/MCP path actually reaches it (commit `451f9730`, per `AnchorPlatformSelectorPropagationInvestigation_2026-08-31.md`). *Update 2026-08-31: re-confirmed with a genuinely different client — `lenovo-client-1` (separate physical LAN, real NAT), all six baseline validators pass again including `DnsFailclosed`, ruling out same-subnet coincidence.* ★ **MILESTONE 2026-08-31 (`labrun-1788266019601-1574-3`, commit `451f9730`): the three `MacosAnchor*` validator stages DISPATCHED for the first time in project history** (they were `absent`/never-planned before): `deploy_macos_anchor_profile`=**pass**, `validate_macos_anchor_bundle_pull`=**pass**, `anchor_validation`=**pass** (grading real delegated evidence, no longer a runtime skip). The fourth, `validate_macos_anchor_port_mapping_authority`, **fails with a real, specific defect** (not a skip): *"macOS anchor macos-utm-1 (node_id=macos-client-1) does not hold `anchor.port_mapping_authoritative` in its membership entry; current election authority=Some(debian-headless-4-bootstrap)"* — but the reported `node_id=macos-client-1` is not even a real identity in this mesh, it is the inventory *label*. **CORRECTED 2026-09-05 (QH-68): this was NOT the "same family" as the exit-cell membership/role gap — it was a validator identity bug.** The stage passed the inventory label as `--self-node-id`, so the validator looked up a nonexistent node and reported its missing capabilities. Fixed in `874a9aaa` (the stage now resolves the real bootstrap identity); **live re-proof PASSED 2026-09-05** (`livelab-1788635706-1eb6e800c4cf`, commit `1eb6e800`, clean, report `state/live-lab-macos-anchor-portmap2-20260905-190047`, `--skip-linux-live-suite --linux-backend linux-wireguard-userspace-shared`): 20 pass / 0 fail / 2 legit skips with all four anchor stages green — `deploy_macos_anchor_profile`, `validate_macos_anchor_bundle_pull`, `anchor_validation`, `validate_macos_anchor_port_mapping_authority` — against the daemon-reported identity `macos-utm-1-bootstrap` (row: `macos_stage_anchor=pass`). The earlier text calling it the same family as `MacosExitMembershipRoleFixDesign_2026-08-31.md` was wrong and is retracted. (Getting the row recorded needed one more fix: the anti-shrink verifier's expected-plan reconstruction read only the always-empty `--anchor-platform` selector on `--node` runs and rejected the faithful plan at finalization — fixed in `1eb6e800`, same commit as this run.) (Run evidence lives in the report dir `state/deepseek-lab-labrun-1788266019601-1574-3/`; the detached `ai_lab_run` orchestrator did not auto-append the tracked run-matrix row — a known workspace-root-resolution gap, flagged for follow-up.) | ⬛ never exercised |
...
| **role-transition** | 🟢 **GREEN 2026-09-07** (`livelab-1788752486-6d3fea6cf2aa`, commit `6d3fea6c`, clean, report `state/live-lab-macos-roleswitch-20260907-032942`, topology macos-utm-1:client / debian-headless-4:exit / debian-headless-2:client, `--role-switch-platform macos --skip-linux-live-suite --linux-backend linux-wireguard-userspace-shared`): `validate_macos_role_transition` hard-passes on the `--node` engine (17 pass/0 fail/3 skip, ledger row appended). Two orchestrator bugs had to fall first — the stage passed live four times before any run could carry its ledger row, because evidence finalization rejected every faithful run: the finalizer's independent plan reconstruction (`verify_recorded_plan_not_shrunk`, `resolved_plan.rs`) never derived the C6/C7 role-switch/reboot elections from the manifest selectors, and the runner's manifest snapshot (`native.rs` manifest_selectors) hardcoded `role_switch_platform`/`reboot_platform` to empty strings, so the reconstruction could never see them — fixed in `61983773` + `6d3fea6c` (regression-pinned by `verify_honors_role_switch_and_reboot_selector_elections` / `verify_rejects_a_fabricated_role_switch_election_without_a_selector`). Same bug class as the MAC-D1 anchor-finalizer fix in the same file. | ⬛ never run on `--node` |
