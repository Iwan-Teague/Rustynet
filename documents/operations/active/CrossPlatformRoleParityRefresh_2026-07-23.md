...
| **anchor** | 🟡 **MAC-D1 rerun CONFIRMED 2026-08-31** (`livelab-1788165016205-17194-3`, commit `5db953ad`, clean, 16 pass/0 fail/3 skip). `bootstrap_hosts` → `validate_baseline_runtime` all pass, and critically **all six baseline validators pass on macos-utm-1 including `DnsFailclosed`** (RuntimeAcls/ServiceHardening/KeyCustody/Authenticode/MeshStatus/DnsFailclosed all `passed:true` — the DnsFailclosed enforcement gap [[exit row below]] evidently only bites in the exit/protected-DNS-mode posture, not plain anchor). `anchor_validation` itself still `skip`s ("no node executed this validation; 1 node(s) reported a runtime skip") — **CORRECTED 2026-08-31, this is NOT a posture-gate issue** (the earlier attribution to `is_supported_for_platform` was wrong; that gate is deliberately never consulted by this stage, per MAC-D1's own de-circularisation). Root-caused in `AnchorValidationMacosPostureGateInvestigation_2026-08-31.md`: `--skip-linux-live-suite` drops the three macOS anchor validator stages (`deploy_macos_anchor_profile`, `validate_macos_anchor_bundle_pull`, `validate_macos_anchor_port_mapping_authority`, all tagged `@ Live`) out of the plan entirely, so MAC-D3's election-tightening check (`native.rs:376-383`) correctly falls back to `false` and the stage correctly fail-closed-skips — but this means the macOS anchor cell can **never** be exercised on the fast path, contradicting the documented `skip_linux_live_suite` contract ("the mac/win stages stay fully exercised"). Fix proposed (plan-builder suite filter needs to be anchor-election-aware), not yet implemented. `live_anchor` was not dispatched (`--skip-linux-live-suite`). See MacCellsHarvest §2. *Update 2026-08-31: re-confirmed with a genuinely different client — `lenovo-client-1` (separate physical LAN, real NAT) in place of a same-host UTM Debian client (`livelab-1788187836531-17194-20`, clean). All six baseline validators pass again including `DnsFailclosed`, ruling out "same-subnet coincidence" as an explanation for the earlier clean anchor result; the anchor/exit DnsFailclosed split is a genuine role-posture difference, not a topology artifact.* | ⬛ never exercised |
