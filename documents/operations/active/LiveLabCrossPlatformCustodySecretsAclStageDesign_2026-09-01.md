...
**Date:** 2026-09-01
**Status:** DESIGN ONLY — no code changed in this document's preparation. Adversarial review COMPLETED 2026-09-01; verdict **READY-WITH-AMENDMENTS**; all review amendments are folded into this document in place. Implementation remains owner-gated and lab-pending: it must not begin until the live lab is available for proving. All live-proving evidence in this document is explicitly marked as pending lab availability; no stage described below has been live-proven on macOS or Windows.

**Adversarial-review amendments folded (2026-09-01):** A1 — rejection is restart-based, not a nonexistent custody watcher: a daemon still running under the downgraded key at assertion time is a failure; macOS recovery = launchd restart + `macos-key-custody-check`, Windows = native argv-only ACL helper + service restart + `windows-key-custody-check`. A2 — DPAPI probe discipline: probe the custody-check status + hash/size only, never decrypt, no key material logged. A3 — the secrets pattern set is pinned (64-hex WireGuard keys, 32-hex ed25519 keys, base64 EC-key headers per `live_linux_secrets_not_in_logs_test.rs`); checked shapes are explicit and additional shapes are new scanner requirements. A4 — an absent-secret result with no generated stimulus is a reported limitation, never a pass (the Linux binary's generate-only stimulus path was removed as a false-pass mode). A5 — doc-drift prerequisite recorded: the stale reported-skip doc comments in `key_custody_validation.rs:19–21` / `runtime_acls_validation.rs:17–20` must be corrected in the implementation change. A6 — citation corrections: the dispatch-table range reads "532 onward" and module paths carry the `orchestrator/` segment. A7 — the Windows §4.7 gating stays reported-skipped behind the `status`-subcommand gap; the identity challenge fails closed (`node_adapter.rs:463–479`, `525–530`).
...
- `crates/rustynet-cli/src/vm_lab/orchestrator/adapter/node_adapter.rs:532 onward` — the `RoleValidatorKind` × platform dispatch table routes `RuntimeAcls` and `KeyCustody` for **all three** desktop platforms to the `validate_*` functions above.
- `crates/rustynet-cli/src/vm_lab/orchestrator/adapter/node_adapter.rs:259–272` — the default `supports_role_validator` returns `true` for Linux | macOS | Windows for every kind except `GossipConvergence` (which is gated by `gossip_convergence_runtime_implemented`). This is the **only** definition in `adapter/`; no per-adapter override exists.
- Therefore the per-node stages `KeyCustodyValidationStage` (`orchestrator/stage/key_custody_validation.rs`, fanout `PerNode`, reported-skips contract at lines 58–61 and 90–107) and `RuntimeAclsValidationStage` (`orchestrator/stage/runtime_acls_validation.rs`, fanout `PerNode`, reported-skips contract at lines 57–60 and 97–106) **execute** the macOS/Windows posture validators on mac/win nodes today. A mac/win node is not silently skipped at this layer; it runs the daemon check and the result is evaluated fail-closed.
...
1. **Adversarial key custody is Linux-only.** `crates/rustynet-cli/src/vm_lab/orchestrator/stage/live_key_custody_validation.rs:48–67` unconditionally runs:
...
2. **Secrets-in-logs validation is entirely Linux-only — no mac/win variant exists at any layer.** `crates/rustynet-cli/src/vm_lab/orchestrator/stage/live_secrets_not_in_logs_validation.rs:45–64` unconditionally runs `--bin live_linux_secrets_not_in_logs_test` against the client node with report `live_secrets_not_in_logs_report.json` and log `live_secrets_not_in_logs.log`. Unlike custody and ACLs, there is **no** platform posture validator for secrets-in-logs either: `orchestrator/role_validation/` contains no secrets module (directory listing checked: admin_issue, anchor, authenticode, blind_exit, blind_exit_dataplane, dns_failclosed, exit_demotion_residue, exit_dns_failclosed, exit_nat_lifecycle, gossip_convergence, identity_challenge, ipv6_leak, key_custody, mesh_status, mod.rs, relay, runtime_acls, security_audit, service_hardening), and no per-platform secrets validator exists anywhere in `vm_lab`. `RoleValidatorKind` (`node_adapter.rs:75–86`) has no `SecretsNotInLogs` variant.
...
- `orchestrator/stage/key_custody_validation.rs:19–21` claims macOS/Windows nodes are "reported-skipped" behind a `key_custody_runtime_implemented` gate.
- `orchestrator/stage/runtime_acls_validation.rs:17–20` makes the same claim behind `runtime_acls_runtime_implemented`.
...
3. **Expect rejection — restart-based, not watcher-based (amendment A1).** There is **no daemon-side custody watcher** continuously watching the key file; the daemon does not self-heal a permission downgrade while running. The rejection semantics are: restart the daemon under launchd after the downgrade, then assert the post-restart state — either the daemon refuses to come up cleanly under the downgraded key or the follow-up `rustynetd macos-key-custody-check` reports a `drifted` status (the same rejection semantics the Linux binary asserts). **A daemon still running under the downgraded key at assertion time is a failure, not a pass** — "no crash observed" is never acceptance evidence. The restart is performed via the node's launchd service unit (stop/start through the existing lab SSH plane, argv-only); the restart itself is recorded in the report (pre/post timestamps, observed service state).
4. **Expect recovery (amendment A1).** Recovery is explicit and test-driven, identical in spirit to the Linux binary's recovery phase: the test itself restores the key-file mode to the required mode (recording pre/post), restarts the daemon again under launchd, and asserts a final `rustynetd macos-key-custody-check` reports `overall_ok = true` with the daemon live under the restored key. Recovery claimed without a post-restore custody-check pass is a failure.
...
2. **Attack A (ACL downgrade; amendment A1):** grant `Users` read on the key file via the native ACL helper. There is **no custody watcher**: the rejection is asserted restart-based — restart the Windows service, then assert the post-restart state (service refuses to come up cleanly under the downgraded ACL, or the follow-up `rustynetd windows-key-custody-check` reports `drifted`). **A daemon still running under the downgraded key at assertion time is a failure, not a pass.** Recovery is explicit and test-driven: restore the ACL through the same native argv-only helper, restart the service again, and assert a final `windows-key-custody-check` reports `overall_ok = true`.
3. **Attack B (DPAPI custody probe, passive; amendment A2 — probe discipline):** copy (not move) the DPAPI-wrapped key blob to a temp path, then assert the daemon's next custody check still reports the service-profile binding intact. The probe observes **status + hash/size only** — the test never attempts to decrypt the copy, in or out of the service context (the blob is DPAPI-bound to the service account profile; "a copy is inert" is evidenced by the custody check's binding status plus the copy's unchanged hash/size, not by any decryption attempt). No key material — plaintext, blob, derived hash, or size beyond the recorded digest/byte-length — is emitted to logs; only hashes/sizes and status booleans enter the report.
...
- Add a `SecretsNotInLogs` variant to `RoleValidatorKind` (`node_adapter.rs:75–86`) with per-platform dispatch entries routing to new `orchestrator/role_validation/secrets_not_in_logs.rs` functions `validate_linux_secrets_not_in_logs`, `validate_macos_secrets_not_in_logs`, `validate_windows_secrets_not_in_logs` (Linux included so the posture layer is symmetric with the existing live Linux binary).
- Each dispatches a new daemon self-check subcommand (`rustynetd secrets-not-in-logs-check`) that scans the daemon's own log sink for the repo's forbidden secret patterns — **the same pinned set §5.2 amendment A3 fixes: 64-hex WireGuard key shapes, 32-hex ed25519 key shapes, and base64 EC-key header blocks** — and reports per-pattern booleans plus `overall_ok`, using the same report schema discipline (schema version, per-entry status, `overall_ok` consistency) the custody/ACL evaluators enforce.
...
- A new `SecretsNotInLogsValidationStage` (fanout `PerNode`, reported-skips contract identical to the two existing posture stages) registers in `plan.rs` / `orchestrator/stage/mod.rs` beside its siblings.
...
1. Generate a marker-bearing trigger through the control path (the same benign secret-shaped stimulus the Linux binary uses). **Amendment A4:** the Linux binary's original generate-only stimulus path (its binary comment, lines 113–118) was removed as a false-pass mode — a scan that finds nothing because no stimulus was ever generated is a **reported limitation, never a pass**. The per-platform tests therefore record per-check whether the stimulus was generated and the pathway driven; a check that could not be exercised is named in the report as a limitation (contributing to `Skipped`/`Failed` per the stage's outcome contract), never folded into `overall_ok = true` by absence.
2. Exercise the daemon pathways most likely to log: enrollment attempt, peer handshake, role transition rejection.
3. Fetch the daemon log (macOS: launchd-managed log path; Windows: the service's log sink) and assert no forbidden pattern appears. **Amendment A3 — pinned pattern set.** The forbidden shapes are exactly the ones `live_linux_secrets_not_in_logs_test.rs` scans for today, and this design pins them explicitly: **64-hex-character WireGuard private-key shapes, 32-hex-character ed25519 key shapes, and base64 `-----BEGIN ... EC PRIVATE KEY-----`-style key-header blocks.** The daemon self-check (`secrets-not-in-logs-check`, §5.1) scans for this same pinned set. Any additional shape (enrollment-token formats, share/preshared-key fields beyond these) is a **new scanner requirement**: it must be added to this pinned set deliberately, with its own detection test, not absorbed silently.
...
| `RoleValidatorKind::SecretsNotInLogs` + `orchestrator/role_validation/secrets_not_in_logs.rs` + daemon `secrets-not-in-logs-check` + evaluators | dispatch table `node_adapter.rs:532 onward`; new `SecretsNotInLogsValidationStage` | posture stage: `ServiceHardeningValidation` (mirroring `KeyCustodyValidationStage`) | `PerNode` |
...
3. **Windows control CLI `status` subcommand** — unblocks the §4.7 identity challenge; until then Windows live tests are implemented-but-reported-skipped (§4.3). **Amendment A7 (review confirmation, kept):** this gating stays exactly as designed — reported-skipped behind the `status`-subcommand gap, with the identity challenge failing closed (`orchestrator/adapter/node_adapter.rs:463–479, 525–530`).
4. **Doc-comment correction (amendment A5 prerequisite)** — the stale mac/win reported-skip claims in `orchestrator/stage/key_custody_validation.rs:19–21` and `orchestrator/stage/runtime_acls_validation.rs:17–20` (§1.3) are corrected in the implementation change that lands the first of the new validators.
