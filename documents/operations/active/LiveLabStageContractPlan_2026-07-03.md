...
| 4 | Shared recorder (`ops record-stage-start/finish`) | **PARTIAL** (working tree, pending commit) — the "every planned enabled stage ends with a terminal outcome" invariant now holds by two mechanisms, even without the full start/finish recorder CLI: (a) the never-dispatched Rust state-machine dialect (`membership_init`, `active_exit`, `role_switch_matrix`, … 13 stages, `state_machine_only` in the registry) is emitted `enabled: false` + skip_reason `"inactive orchestrator dialect …"` instead of pending-and-expected; (b) `reconcile_barrier_exempt_outcomes` (in `finalize_vm_lab_orchestration_result`) appends explicit `skipped` rows into `orchestrate_result.json` for every enabled, non-synthetic, barrier-exempt stage that recorded nothing (job pseudo-stages `vm_lab_setup_live_lab`/…, conditional infra `restart_unready_vms`/`rediscover_local_utm`) — verdict-neutral (runs after overall_result + CSV are locked) and honest (the stage did not dispatch this invocation). Verified against 129 recorded runs: the dead dialect had recorded in 0 of them. Still open: the shared start/finish recorder CLI and per-stage recorder validation (Finding 1C/10). |
