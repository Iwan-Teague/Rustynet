...
| 4 | Shared recorder (`ops record-stage-start/finish`) | **PARTIAL** (831a32a; earlier half 844175f) — the "every planned enabled stage ends with a terminal outcome" invariant + the first realtime recorder now hold. (a) The never-dispatched bash-path Rust dialect (13 `state_machine_only` stages) is emitted `enabled:false` + `"inactive orchestrator dialect …"` skip_reason; `reconcile_barrier_exempt_outcomes` (in `finalize_vm_lab_orchestration_result`) backfills explicit `skipped` rows for enabled non-synthetic barrier-exempt stages that recorded nothing (job pseudo-stages, conditional infra) — verdict-neutral (after overall_result + CSV). (b) **NEW recorder module `live_lab_stage_recorder.rs`** — the single owner of `state/stages.tsv`: `record_stage_start` upserts a `running` row, `record_stage_finish` replaces it with the terminal outcome (atomic tmp+rename, 8-col v1 layout + `#schema_version=2` marker, single-writer). (c) **The Rust `--node` path now emits the full contract** (831a32a): `build_stage_manifest` gained an `active_plan` override (bash byte-identical on `None`; Rust dialect enabled + **barrier-eligible** on `Some(plan)`); `StateMachineRunner::run_with_observer` + a `StageObserver` seam drives realtime `stages.tsv` rows; and `execute_rust_native_orchestration` emits the plan manifest before running and routes conclusion through `finalize` (new inventory-preserving variant) for `orchestrate_result.json` + reconcile + a SINGLE run-matrix row (killed the degenerate-key double-write). A `--node` run therefore renders in today's unchanged monitor. Still OPEN: the bash path calling the recorder (per-stage `ops record-stage-start/finish` subcommands), the monitor reading active-stage from the running row (demoting its inference heuristics), `report_state` job_state/heartbeat, and per-stage recorder validation (Finding 1C/10). |
