...
| RNQ-02 | Code complete; live negative proof pending | Final cleanup now calls `assert_node_clean` after cleanup. Linux/macOS include daemon + relay + interface + firewall/NAT probes; Windows includes daemon service + relay service + adapter + firewall + NAT. Closure test proves detected residue fails. Per-OS live residue fixtures remain. |
...
| RNQ-05 | Substantially implemented; fault-injection matrix pending | Recorder/manifest/summary/parity/report-state/artifact/finalizer errors now block pass, and matrix append failure is fatal. Existing manifest/stage writers are atomic. Exhaustive writer fault injection and a single fsync-backed multi-artifact transaction remain. |
...
| RNQ-15 | Partial | Readiness and failure diagnostics moved out of `vm_lab/mod.rs` into `orchestrator/readiness.rs` and `orchestrator/diagnostics.rs`; bounded fanout is isolated in `parallel.rs`. Native executor and evidence/finalization blocks still require extraction. |
| RNQ-16 | Partial | One macro now owns `StageId` variants, canonical order, and wire names. Six posture stages dispatch through typed `RoleValidatorKind` behind `NodeAdapter`. Registry metadata, plan construction, and historical oracle generation remain separate authorities. |
| RNQ-17 | Open | A safe product/lab crate split requires moving the interleaved main CLI parser/dispatch and release/SBOM packaging. No cosmetic wrapper was added because it would leave the product attack surface unchanged. |
