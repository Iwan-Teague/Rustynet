...
| RNQ-05 | Commit-marker-last finalizer + per-writer fault injection implemented in `evidence.rs`; **2026-07-19: production wiring DONE** (`native.rs` calls `finalize_rust_native_run`, inline block deleted) + pass-path pre-marker durability barrier (`sync_report_dir_tree`); live green `--node` re-proof pending | Recorder/manifest/summary/parity/report-state/artifact/finalizer errors now block pass, and matrix append failure is fatal. **2026-07-12 fix (`0cbf98d`):** a fully-green fresh full `--node` run failed finalization and appended NO matrix row — the reuse-evidence seal (attempted only when `candidate_pass`) digests `state/orchestration_context.json`, which a fresh full run never persisted (only `--setup-only` did; `--run-only` loaded it), so it failed `No such file or directory`; a *failing* run skipped the seal and therefore DID record a row, so only failing runs ever appeared in the matrix. Now the full-run path persists the context (atomic+fsync) before the seal; failure demotes via `evidence_errors`. Live-proven by a green run appending its row. **2026-07-13:** (1) the shared atomic temp+fsync+dir-fsync primitive is generalized (`context.rs::atomic_write_fsync`) and `report_state.json` now commits through it (`write_report_state_durable`), so the run's commit marker is crash-safe/untearable in the live path immediately. (2) A single-transaction finalizer `evidence.rs::finalize_rust_native_run` orders every artifact durable-first and makes `report_state.json run_passed=true` the LAST fsync'd write (single commit marker); a crash before it leaves the run non-passed. (3) A test-only fault seam (`EvidenceWriter`/`set_evidence_fault`) drives one demotion test per writer (run-summary, failure-digest, parity, artifact-completeness, context-persist, reuse-seal, matrix-append, commit-marker) + a prior-recorder-error test + a green ordering test proving the marker is written strictly after the matrix append (10 tests, all green). Remaining: coordinator wires `native.rs`'s inline finalization block to call `finalize_rust_native_run` (passing the real `finalize_vm_lab_orchestration_result_with_inventory` as the `matrix_finalize` closure) so the marker-last ORDERING is active in the production path, then a live green `--node` run re-proves the matrix row. |
...
| RNQ-09 | Code complete; subprocess signal proof landed 2026-07-14 | Fatal SIGTERM/SIGINT registration now precedes readiness/inventory mutation. Injectable registration-failure test proves following readiness work is not entered. **Real subprocess SIGTERM/SIGINT cleanup test added:** `src/bin/rnq09_signal_harness.rs` (test-only, whole-file `#![cfg(feature = "vm-lab")]` + `required-features = ["vm-lab"]` in `Cargo.toml` — a `cfg` gate alone is not sufficient to keep a binary target's default build green, since a cfg'd-out crate still needs a `fn main`) drives the REAL `register_shutdown_handlers` + `StateMachineRunner::with_shutdown_flag` seam (via the RNQ-17 `orchestrator_test_surface` re-export) inside an actual OS process: it registers handlers first (mirroring `native.rs`), prints a flushed `READY` handshake, then a stage cooperatively polls the shutdown flag while mid-execution. `tests/rnq09_signal_cleanup.rs` spawns it, blocks on the `READY` line (a real blocking read, not a sleep — the anti-flake mechanism), delivers a real `kill -TERM`/`-INT` to its pid, then asserts the fail-closed contract: non-zero exit, the `always_run` cleanup stage's marker file exists, the downstream must-skip stage's marker never exists, and no `PASSED` marker is ever written on any path. Two tests (SIGTERM, SIGINT), both green; 20/20 in a local stress loop. Unblocking this required extending `orchestrator_test_surface` (RNQ-17) with `OrchestrationStage` + `StageFanout`, which the harness needs to define its own synthetic stages — both were already `pub` inside the vm_lab tree. **Bonus gap closed:** `with_shutdown_flag` had zero coverage at any level before this; a new deterministic in-process unit test in `orchestrator/runner.rs` (`shutdown_flag_skips_pending_non_always_run_stage_but_always_run_cleanup_still_executes`) pre-sets the flag and proves, via an execution counter (not just outcome equality), that a pending non-`always_run` stage's `execute()` is never called while `always_run` cleanup still runs exactly once. Verified: `cargo fmt --all -- --check`, default `cargo check -p rustynet-cli` (no features, confirms the harness bin is fully skipped), `cargo clippy -p rustynet-cli --features vm-lab --all-targets -- -D warnings` (zero warnings), and `cargo test -p rustynet-cli --features vm-lab` full suite green (a single unrelated pre-existing flake was seen once in `source_archive_marker_matches_worktree_snapshot_commit` — root-caused to `git stash create` producing a timestamp-dependent SHA when called twice under heavy parallel load; untouched by this change, 15/15 green in isolation, and green on suite re-run). |
...
| RNQ-17 | Implemented 2026-07-14 (option (a) feature-gate) | Landed as the DEFAULT-OFF cargo feature `vm-lab` on `rustynet-cli`. Conditionally compiled behind it: `mod vm_lab;` + 10 lab helper modules (`ops_live_lab_orchestrator`, `ops_cross_network_preflight`, `ops_fresh_install_os_matrix`, `ops_live_lab_failure_digest`, `live_lab_{coverage,results,run_matrix,stage_manifest,stage_recorder,stage_registry}`), 120 lab `OpsCommand` variants + their parser/dispatch arms, and the lab half of `help_text()`. `zip`/`toml`/`socket2`/`signal-hook` are now optional deps pulled only by `vm-lab`. Two lab-adjacent modules stay compiled by design: `ops_e2e` (product paths call it — `refresh_local_traversal_bundle_from_specs` bundle issuance + the Windows relay/exit role-transition service actions in `main.rs`) and `ops_cross_network_reports` (`ops_phase9`'s phase10-readiness verifier calls its validators; gating would weaken a release-readiness check) — their lab-facing command surface IS gated. The shared secret/nonce helpers moved verbatim from `main.rs` to `src/secret_material.rs` so the library can compile `ops_e2e`. `lib.rs` gained the `#[cfg(feature = "vm-lab")]` `orchestrator_test_surface` re-export block (RNQ-09 unblock): `StateMachineRunner`/`StageObserver`/`StageId`/`StageOutcome`/`NodeRole`/`OrchestrationContext`/`register_shutdown_handlers{,_with}` (later extended with `OrchestrationStage`/`StageFanout` when the RNQ-09 subprocess harness needed to define its own synthetic stages — see the RNQ-09 row). All lab-driving call sites updated to pass the feature: guest builds (`rn_bootstrap.sh`, `Bootstrap-RustyNetMacos.sh`, `vm_lab/mod.rs` toolchain build, `ops_e2e.rs` bootstrap script), the bash orchestrator + cross-network/single-stage wrappers, `probe_and_recover_local_utm.sh`, `diff_lab_runs.sh`, `anchor_live_lab_gates.sh` (its `vm_lab::tests::*` filters would otherwise silently match zero tests), 7 lab-driving `src/bin` gate/e2e binaries, and the `rustynet-mcp` lab-state/deepseek spawn vectors. Verified: default `rustynet --help` exposes no vm-lab/live-lab/e2e/cross-network/fresh-install commands; `cargo tree -e no-dev` default has no zip/toml (socket2/signal-hook remain only as transitive deps of other crates); `--all-features` gates green. |
...
- **Status 2026-07-13:** The transaction + fault injection are implemented in
  `crates/rustynet-cli/src/vm_lab/orchestrator/evidence.rs`
  (`finalize_rust_native_run`) over the generalized atomic primitive
  `crates/rustynet-cli/src/vm_lab/orchestrator/context.rs::atomic_write_fsync`,
  with `report_state.json run_passed=true` as the last fsync'd write and a
  per-writer fault-injection test proving demotion at every writer (see the
  RNQ-05 status row for the writer list). The commit marker is already durable
  in the production path (`write_report_state_durable`); the marker-last
  ordering becomes active once `native.rs`'s inline finalization block is
  rewired to call `finalize_rust_native_run`.
- **Status 2026-07-19 — production wiring DONE + durability barrier added.**
  (1) `native.rs::execute_rust_native_orchestration` now calls
  `finalize_rust_native_run` (passing the real
  `finalize_vm_lab_orchestration_result_with_inventory` as `matrix_finalize`);
  the ~150-line inline finalization block — which wrote `run_passed=true`
  BEFORE the matrix append and demoted on a discarded `Result` — is DELETED,
  so the transaction is the single finalization path (§3 one-hardened-path)
  and the marker-last ordering is live in production. The dead-code allows on
  the finalizer surface are removed; `--setup-only`/`--run-only` context
  semantics are preserved (binding passed only for a full run; a binding that
  cannot be built demotes via `prior_evidence_errors`). (2) New step 9
  **durability barrier** (`sync_report_dir_tree`): on the pass path only,
  after the matrix append and strictly before the commit marker, every
  regular file and directory under the report dir is fsync'd (symlinks never
  followed; unix-only, matching `atomic_write_fsync`'s dir-fsync precedent).
  Rationale: the marker's own fsync flushes nothing else — most evidence
  writers (stage logs, `nodes.tsv`, summaries, manifest/rows,
  `orchestrate_result.json`) are plain or rename-only writes, so a power cut
  after the marker could keep a durable `run_passed=true` over torn evidence,
  which is the literal partial pass the finding forbids. A barrier failure
  demotes (marker written false, `Err` returned). New
  `EvidenceWriter::DurabilityBarrier` fault-injection test + the sharpened
  helper (post-matrix writers now positively assert the append happened);
  the green ordering test now also exercises the real fsync walker. 12/12
  finalize/evidence tests green; `cargo check -p rustynet-cli
  --all-features` green; my-diff files clippy-clean (5 pre-existing clippy
  findings in untouched files: `live_lab_run_matrix.rs:2593`,
  `live_lab_stage_triage.rs:212`, `vm_lab/mod.rs:4653/6792/7187` — newer
  local clippy 1.97 vs pinned 1.88, outside this change). REMAINING for full
  closure: a live green `--node` run re-proving the matrix row through the
  wired transaction (next lab access), and the broader §14 ambition (real
  disk-full/short-write OS-level fault injection) stays open repo-wide.
...
- **Resolution (2026-07-14):** Implemented as option (a) — the default-off
  `vm-lab` cargo feature on `rustynet-cli` (see the §2 status row for the full
  shape). The release workflow builds with no features, so the shipped binary
  drops the lab command surface and lab-only deps without a crate split; the
  crate-split remains available later if the shared-library boundary needs it.
