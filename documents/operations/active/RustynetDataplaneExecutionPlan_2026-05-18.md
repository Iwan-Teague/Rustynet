...
- **Depends on.** D2.5, D4.
- **Status (2026-05-20).** **Complete (end-to-end).** Commit d2412ee: operator-facing surface and daemon-side IPC consume land on top of the 0a54818 primitives. (1) New `inspect_token` non-mutating verify helper, versioned `load_ledger`/`write_ledger` spool, and strict-permissions `load_secret`/`write_secret` (32-byte file, 0o600). (2) New `enrollment_consume.rs` orchestrator atomically validates the HMAC tag, persists the ledger, and registers the enrollee in `GossipNode.peers` — ledger persisted BEFORE peer registration so a torn spool write aborts before the in-memory table mutates. `PushAddressPolicy::Strict` enforces the Global/Private scope filter in production; `AllowLoopback` is test-only. (3) `IpcCommand::EnrollmentConsume { token, pubkey_b64, push_addr }` with matching wire form. (4) CLI `rustynet enrollment {mint, verify, consume}`: Mint and Verify run locally against a secret file the operator owns; Consume goes through the daemon IPC socket. (5) Daemon wiring: `--enrollment-secret` / `--enrollment-ledger` CLI flags, matching env vars, and entries in `scripts/systemd/rustynetd.service`. The §D2.7 pass criterion is pinned by `crates/rustynetd/tests/enrollment_two_peer_redeem.rs::enrollee_joins_mesh_via_token_consume_and_participates_in_gossip`: existing peer A mints, new device N is consumed into A's peer table, A's mint reaches N within 3 s, AND N mints back so A applies N's endpoints within 3 s — proving N is a participating peer. Four negative pins follow (replay → AlreadyConsumed, wrong-secret → TagMismatch + no peer registered, expired → Expired, loopback push address → UnreachablePushAddress without burning the token). 13 new unit tests across the helpers. Production-side membership-bundle co-signing (folding the enrollee's pubkey into the next signed membership snapshot for cross-peer trust propagation) is the natural next slice; the current code path takes the enrollee's verifying key at face value and adds them to A's local gossip routing table, which is sufficient for the §D2.7 pass criterion but does not yet propagate the new identity to A's other peers.
