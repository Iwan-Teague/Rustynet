...
- Status: **applied** (2026-06-24, option A — derive from the signed record). `reduce_membership_state` now takes `op_created_at_unix` and stamps the affected node's `updated_at_unix` with it instead of `unix_now()`; `preview_next_state` threads it (the proposer passes the same `created_at_unix` the record carries) and `apply_signed_update` re-derives with `record.created_at_unix`. The `state_root` is now a pure function of (state, operation, signed-timestamp), so RevokeNode/RestoreNode/RotateNodeKey/SetNodeCapabilities reproduce their recorded `new_state_root` at apply time and apply successfully. All 11 `preview_next_state` callers (cli/rustynetd/control + tests) pass the recorded timestamp; the now-unused `unix_now()` was removed. AddNode/RemoveNode/SetQuorum/RotateApprover are unchanged (they never used `unix_now()`), so existing valid state-roots are unaffected. Tests: `rsa0009_{revoke,restore,rotate_key,set_capabilities}_applies_when_created_at_differs_from_apply_time` (build at created_at=120, apply at now=130 → success) and the strengthened `revoke_and_restore_update_timestamp`. Chosen over (C) excluding from canonical_payload (would change AddNode's existing state-roots) and (B) a new op field (wire change); (A) preserves the updated_at_unix-tracks-changes intent the existing test encodes.
...
- Status: **applied** (2026-06-24). `ensure_homebrew` now registers `trap 'rm -f "${sudoers_tmp}"' EXIT` immediately after writing the temporary `NOPASSWD` grant and before the `curl|bash` installer, clearing it (`trap - EXIT`) on the success path — mirroring the script's existing scoped-trap idiom — so a failed/aborted/interrupted bootstrap removes the grant on every exit path. Lint test `bootstrap_removes_temporary_sudoers_grant_on_every_exit_path` asserts the trap is registered between the grant write and the curl|bash and cleared on success. (The over-broad `ALL=(ALL) NOPASSWD: ALL` scope is retained — narrowing it to Homebrew's individual sudo calls is fragile; the residue, which was the actual vuln, is closed.)
...
- Status: **APPLIED** (2026-07-17) — resolved via the helper option, and **both** halves of the proposed enforcement were taken, because either alone leaves a hole.
  - **Helper defined + applied at all four sites.** `secure_remove_file()` now exists in `scripts/bootstrap/macos/Bootstrap-RustyNetMacos.sh`, mirroring `rustynetd`'s `key_material::remove_file_if_present`: `stat -L` before `-f`, unlink a symlink **as** a symlink (never scrub through it), fail closed on a non-regular file, zero-fill, then unlink. Applied at all four removals this finding named — the literal `${KEYS_DIR}/wireguard.passphrase`, the `${passphrase_tmp}` EXIT trap, `${runtime_key}` (the plaintext WG private key), and `${passphrase_file}`. This finding's own warning was heeded: the literally-named line was **not** switched to a variable.
  - **Scanner tightened so variable indirection cannot evade it.** `ops_phase1.rs` gained `line_removes_secret_artifact` / `line_runs_forced_rm`: literal names **or** secret-ish variable tokens, comments skipped, `rm -rf`/`-fr`/`--force` covered, `rm` required at a word boundary, `rm -- -f` treated as an operand. Confirmed by planting `rm -f "${runtime_key}"` — caught at `:1118`, which the old literal-only rule walked past. Blast radius scanned across all tracked `.sh` first: 0 false positives.
  - **Honest limit, unchanged from this finding's analysis:** the zero-fill is best-effort and documented as such in the helper. On APFS/SSD (copy-on-write, wear levelling, snapshots) an in-place overwrite may land on fresh blocks; macOS dropped `srm` and deprecated `rm -P` for that reason. The load-bearing protection is the symlink/type safety, not the scrub — the same promise `remove_file_if_present` makes, so both sides of the codebase now claim the same thing.
  - **Gate restored to green honestly:** `secrets_hygiene_gates.sh` → `Secrets hygiene gate: PASS (18 checks)`, exit 0. Separately, the gate was made to **accumulate** rather than stop at the first red (it had been hiding every check behind this finding); a 2-fault run now reports both with `file:line` and exits 78.
  - Verification: helper unit-tested standalone (absent / empty / regular file / **symlink attack — link unlinked, victim intact** / directory fail-closed / dangling symlink / 1-byte / 0-byte); 14 new Rust tests (8 scanner-rule, 6 exit-code precedence).
- Note (2026-07-17): this entry's sibling-site analysis was correct and load-bearing. Fixing only the flagged `:899` would have turned the gate green while three identical exposures remained — including the plaintext runtime key.
