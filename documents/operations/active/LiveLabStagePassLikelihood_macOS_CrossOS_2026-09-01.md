...
- **Citation:** registry `extended_soak` spec — `EnableRule::SoakSuite`, and `TargetSelectors::resolves` defines SoakSuite as `soak_suite && !skip_linux_live_suite` with the comment "extended_soak only ever dispatches as part of the Linux live suite" (`live_lab_stage_registry.rs:336-346`; the rule itself is `:347`); `live_extended_soak_validation.rs:44-47` additionally requires a second client. ~~**Per-stage rows: zero, on every platform — the stage has NEVER dispatched.**~~ **Corrected 2026-09-02:** per-stage truth (`live_lab_node_stage_results.csv`) is Linux **40 pass / 348 skip**, macOS 15 skip, Windows 2 skip — the stage HAS dispatched and passed on Linux under `--node` (roll-up: `linux_stage_extended_soak` = 8 pass / 113 skip / 121 not_run).
- **Root cause:** ~~**WIRING** — the soak selector is never set by the default plan or the wrapper keys; `--skip-soak` is exposed but no enable flag is wired into the mac/win paths.~~ **Corrected 2026-09-02:** the premise is refuted — soak is in the DEFAULT plan (`plan.rs` `include()`: `StageSuite::Soak => !skip_live_suite && !skip_soak`; `--skip-soak` opts out), so the macOS column reads all-skip only because every mac/win cell runs the `--skip-linux-live-suite` fast path, which drops the whole post-baseline suite by design. There is no missing enable flag; the real macOS gap is a full-suite run electing macOS (with the second client the stage requires), not a flag.
- **Pass-likelihood: MEDIUM** (code looks platform-aware; ~~never exercised anywhere, so first dispatch is an unknown-unknown~~ — corrected: Linux-proven with 40 pass node-rows; the first *macOS* dispatch remains unexercised).
- ~~**Improvement (small):** wire a `--enable-soak-suite` selector through `TargetSelectors`/the orchestrator flags and the mac/win target keys so the stage can at least be elected; expect a triage pass on first run.~~ **Corrected 2026-09-02:** no flag needed — the stage dispatches whenever a full-suite (no `--skip-linux-live-suite`) run includes macOS; budget one full-suite-priced run instead.
...
6. **Small wiring fixes:** ~~`--enable-soak-suite` selector (so the never-dispatched `extended_soak` can run at all)~~ — **Corrected 2026-09-02: retracted** — `extended_soak` already dispatches in the default Linux full-suite plan (40 Linux pass node-rows; the mac/win skips are the `--skip-linux-live-suite` fast path by design); the real gap is one full-suite run electing macOS, not a flag; `cross_os:` field on the `--node` `live_lan_toggle_validation` spec; confirm the `--role-switch-platform macos` `--node` dispatch exists, then elect it → `macos_stage_role_transition`. Each small; each needs a run to verify.
