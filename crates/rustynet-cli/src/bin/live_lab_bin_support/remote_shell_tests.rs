...
fn shell_quote_round_trips_apostrophes_safely() {
    ...
    assert_eq!(super::super::shell_quote("plain"), "'plain'");
    assert_eq!(
        super::super::shell_quote("with 'quote'"),
        "'with '\\''quote'\\'''"
    );
    assert_eq!(
        super::super::shell_quote("$INJECT; rm -rf /"),
        "'$INJECT; rm -rf /'"
    );
    assert_eq!(super::super::shell_quote("` ;|& > <"), "'` ;|& > <'");
}
...
fn new_remote_shell_host_returns_a_macos_backend_for_macos_platform() {
    let host = new_remote_shell_host(
        super::super::LiveLabPlatform::MacOs,
        std::path::PathBuf::from("/tmp/id"),
        std::path::PathBuf::from("/tmp/kh"),
        "mac@host".to_owned(),
    );
...
fn new_remote_shell_host_returns_a_windows_backend_for_windows_platform() {
    let host = new_remote_shell_host(
        super::super::LiveLabPlatform::Windows,
        std::path::PathBuf::from("/tmp/id"),
        std::path::PathBuf::from("/tmp/kh"),
