...
//!
//! Track B Phase 28 introduces this trait as the single hardened seam
//! between the live-lab substage code and per-OS shell-out paths.
//! Before Phase 28, every privileged remote command flowed through
//! [`super::capture_root`], which wraps its body in `sudo -n sh -lc
//! <body>`. That works on Linux + macOS (POSIX shells) but is unusable
//! on Windows (PowerShell, no `sudo`), which forced Windows substages
//! to either skip or reach into [`super::capture_remote_stdout`] as a
//! workaround. The mixed approach made each new substage rediscover
//! the cross-platform quoting rules and audit posture from scratch.
//!
...
pub fn new_remote_shell_host(
    platform: super::LiveLabPlatform,
    identity: PathBuf,
    known_hosts: PathBuf,
    target: String,
) -> Arc<dyn RemoteShellHost> {
    match platform {
        super::LiveLabPlatform::Linux => {
            Arc::new(LinuxShellHost::new(identity, known_hosts, target))
        }
        super::LiveLabPlatform::MacOs => {
            Arc::new(MacosShellHost::new(identity, known_hosts, target))
        }
        super::LiveLabPlatform::Windows => {
            Arc::new(WindowsShellHost::new(identity, known_hosts, target))
        }
    }
}
...
/// match the pre-Phase-28 behaviour exactly. Wraps the existing
/// [`super::capture_remote_stdout`] / [`super::ssh_status`] paths so
/// the trait is a strict superset of `capture_root`.
...
impl LinuxShellHost {
    pub fn new(identity: PathBuf, known_hosts: PathBuf, target: String) -> Self {
        Self {
            identity,
            known_hosts,
            target,
...
impl RemoteShellHost for LinuxShellHost {
    fn read_file(&self, remote_path: &str) -> Result<Vec<u8>, RemoteShellError> {
        posix_read_file(&self.identity, &self.known_hosts, &self.target, remote_path)
    }
...
impl RemoteShellHost for WindowsShellHost {
    ...
    fn tcp_send_recv(
        ...
        .map_err(|err| RemoteShellError::Network {
...
fn posix_read_file(
    ...
    let body = format!(
        ...
        super::shell_quote(remote_path)
    );
    let stdout = super::capture_remote_stdout(identity, known_hosts, target, body.as_str())
        .map_err(|err| RemoteShellError::Transport {
...
fn posix_write_file(
    ...
    super::capture_remote_stdout(identity, known_hosts, target, body.as_str()).map_err(|err| {
...
fn posix_stat(
    ...
    let body = format!(
        ...
        fmt = format_arg,
        path = super::shell_quote(remote_path),
    );
    let stdout = super::capture_remote_stdout(identity, known_hosts, target, body.as_str())
        .map_err(|err| RemoteShellError::Transport {
...
fn posix_run_argv(
    ...
    validate_argv(argv)?;
    validate_env(env)?;
    let argv_quoted: Vec<String> = argv.iter().map(|s| super::shell_quote(s)).collect();
    ...
    let body = format!(
        ...
        stdin_b64_quoted = super::shell_quote(&stdin_b64),
    );
    let stdout = super::capture_remote_stdout(identity, known_hosts, target, body.as_str())
        .map_err(|err| RemoteShellError::Transport {
...
fn posix_tcp_send_recv(
    ...
    let body = format!(
        ...
        payload_b64_quoted = super::shell_quote(&payload_b64),
        host = super::shell_quote(host),
        port = port,
    );
    let stdout = super::capture_remote_stdout(identity, known_hosts, target, body.as_str())
        .map_err(|err| RemoteShellError::Network {
...
fn windows_run_powershell(
    ...
    super::capture_remote_stdout(identity, known_hosts, target, command.as_str()).map_err(|err| {
        RemoteShellError::Transport {
