...
fn run() -> Result<(), i32> {
    run_ops_with_passthrough("collect-platform-parity-bundle")
}
...
fn run_ops_with_passthrough(ops_subcommand: &str) -> Result<(), i32> {
    ...
    if status.success() {
        ...
        // Pass through the subprocess exit code so the caller sees the
        // inner cargo / rustynet-cli taxonomy code intact.
