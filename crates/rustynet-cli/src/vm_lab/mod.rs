    ...
    let bootstrap_outcome = if dry_run {
        ...
        let result = (|| -> Result<String, String> {
            ...
            let host_key = if options.skip_access_bootstrap {
                ...
                // administrators_authorized_keys on the Windows guest. A
                // missing or unreadable public key is non-fatal: bootstrap
                // proceeds without key authorization (useful when the key
                // is already present from a prior run).
