...
impl PlanBuilder {
    ...
    pub const LIVE_SUITE_STAGES: [crate::vm_lab::orchestrator::stage::StageId; 23] = {
...
mod tests {
    ...
    #[test]
    fn build_returns_40_stages() {
        let stages = PlanBuilder::new().build();
        assert_eq!(stages.len(), 40, "plan must contain exactly 40 stages");
    }
    ...
    fn skip_live_suite_drops_the_post_baseline_suite_but_keeps_setup_and_cleanup() {
        ...
        // 40 total - 23 live-suite stages = 17.
        assert_eq!(ids.len(), 40 - PlanBuilder::LIVE_SUITE_STAGES.len());
