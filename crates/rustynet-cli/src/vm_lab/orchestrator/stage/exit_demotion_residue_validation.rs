    ...
    fn execute(&self, ctx: &mut OrchestrationContext) -> StageOutcome {
        ...
        if !exit_demotion_residue_runtime_implemented(platform) {
            ...
            return StageOutcome::Skipped(format!(
                "exit-demotion residue validation is not implemented for {platform:?}"
            ));
        }
        ...
        let validation = match validation_kind_for(platform) {
            ...
            None => {
                return StageOutcome::Failed(format!(
                    "{alias}: exit demotion residue gate accepted {platform:?} but \
                     no validator dispatches it (gate/dispatch desync)"
                ));
            }
        };
        let failures = match validation {
            ExitDemotionResidueValidationKind::Linux => {
                match validate_linux_exit_demotion_residue(&*shell, daemon_path, &alias) {
                    Ok(()) => Vec::new(),
        ...
        // failure fails the stage — an unwitnessed demotion-residue pass must
        // never be recorded (the runner would demote it).
        match outcome_for(&failures, &reported_skips) {
            StageOutcome::Passed => {
                match write_exit_demotion_residue_witness(&ctx.report_dir, &alias) {
                    Ok(()) => StageOutcome::Passed,
                    Err(e) => StageOutcome::Failed(format!(
                        "exit demotion residue witness write failed: {e}"
                    )),
                }
            }
            other => other,
...
/// Writes the QH-83 witness line for an exit-demotion-residue PASS: the exit
/// alias whose demotion residue (NAT + firewall rules) was actually proven
/// clean, so a bare PASS can never be recorded without the on-disk evidence
/// behind it. An unwritable witness fails the stage — fail closed.
fn write_exit_demotion_residue_witness(
    report_dir: &std::path::Path,
    exit_alias: &str,
) -> Result<(), String> {
    append_stage_evidence_line(
        report_dir,
        StageId::ExitDemotionResidueValidation.as_str(),
        &format!("exit_demotion_residue=clean exit={exit_alias}"),
    )
}

/// The validator the exit demotion residue proof dispatches. Extracted from
/// `execute` so the dispatch table is unit-testable: the former
/// `_ => unreachable!("runtime implementation gate accepts desktop platforms
/// only")` arm panicked the stage thread whenever the runtime gate and this
/// table drifted (audit I1). An uncovered platform now maps to `None` and
/// the stage fails closed naming the desync instead.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum ExitDemotionResidueValidationKind {
    Linux,
}

fn validation_kind_for(platform: VmGuestPlatform) -> Option<ExitDemotionResidueValidationKind> {
    match platform {
        VmGuestPlatform::Linux => Some(ExitDemotionResidueValidationKind::Linux),
...
mod tests {
    ...
    /// Mutation caught: restoring the `_ => unreachable!("runtime
    ...
    fn validation_kind_never_panics_on_ungated_platforms() {
        assert_eq!(
            ...
            Some(ExitDemotionResidueValidationKind::Linux)
        );
        for uncovered in [
            VmGuestPlatform::Macos,
            VmGuestPlatform::Windows,
    ...
    // QH-83: the pass witness must name the exit alias and the clean verdict
    // — a bare "pass" line would be appeasement, not evidence. Mutation
    // caught: dropping the witness write or demoting it to best-effort.
    #[test]
    fn exit_demotion_residue_witness_line_names_the_exit_alias() {
        ...
        write_exit_demotion_residue_witness(&dir, "exit-a").expect("witness write");
        ...
        .expect("stage log");
        assert!(log.contains("exit_demotion_residue=clean"), "{log}");
    ...
    #[test]
    fn exit_demotion_residue_witness_write_failure_is_propagated() {
        let dir = std::env::temp_dir().join(format!(
            "exdemo_witness_blocker_{}_{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .expect("clock")
        ...
        let err = write_exit_demotion_residue_witness(&dir, "exit-a")
