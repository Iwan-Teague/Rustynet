...
/// Prove every Linux node's daemon passes the runtime-ACLs self-check —
/// canonical runtime root set present, no unexpected additions, per-path
/// status consistent — folding the formerly bash-only check into the standard
/// Rust orchestrator so a `--node` run exercises it.
///
/// Runs after `dns_failclosed_validation` and before the relay/traffic stages.
...
impl OrchestrationStage for RuntimeAclsValidationStage {
    ...
    fn execute(&self, ctx: &mut OrchestrationContext) -> StageOutcome {
        ...
        // Aliases whose runtime-ACLs validator actually ran and passed — the
        // witness names them so a PASS is never backed by nothing.
        let mut validated: Vec<String> = Vec::new();
        for alias in &aliases {
            let adapter = match ctx.adapters.get(alias.as_str()) {
                Some(adapter) => adapter,
                None => {
                    failures.push(format!("{alias}: no adapter for runtime-acls node"));
                    continue;
                }
            };
            ...
            match adapter.run_role_validator(RoleValidatorKind::RuntimeAcls, expected_node_id, None)
            {
        ...
        // failure fails the stage — an unwitnessed runtime-ACLs pass must
        // never be recorded (the runner would demote it).
        match outcome_for(&failures, &reported_skips) {
            StageOutcome::Passed => match write_runtime_acls_witness(&ctx.report_dir, &validated) {
                Ok(()) => StageOutcome::Passed,
                Err(e) => StageOutcome::Failed(format!("runtime acls witness write failed: {e}")),
            },
            other => other,
        }
    }
...
/// Writes the QH-83 witness line for a runtime-ACLs PASS: the validated node
/// count plus each alias whose runtime root-set posture was actually proven,
...
fn write_runtime_acls_witness(
    report_dir: &std::path::Path,
    validated: &[String],
) -> Result<(), String> {
    append_stage_evidence_line(
        report_dir,
        StageId::RuntimeAclsValidation.as_str(),
        &format!(
            "runtime_acls=yes validated_nodes={} ({})",
            validated.len(),
            validated.join(",")
        ),
    )
...
fn reported_skips_json_bytes(reported_skips: &[(String, String)]) -> Vec<u8> {
    ...
    let body = serde_json::json!({
        "stage": "runtime_acls_validation",
        "reported_skipped_runtime_acls": skipped,
        "reason": "Runtime-ACLs check runs live on Linux through the Rust engine; \
...
mod tests {
    ...
    fn outcome_failure_is_failed_even_with_skips() {
        assert!(matches!(
            outcome_for(
                &["deb-1: runtime acls check failed".into()],
    ...
    fn reported_skip_note_names_every_skipped_node() {
        ...
        assert!(s.contains("runtime_acls_validation"));
    }
    ...
    fn empty_assignments_is_skipped_never_passed() {
        ...
        assert!(
            matches!(
                RuntimeAclsValidationStage.execute(&mut ctx),
                StageOutcome::Skipped(_)
            ),
    ...
    // best-effort.
    #[test]
    fn runtime_acls_witness_line_names_the_validated_node_count() {
        ...
        write_runtime_acls_witness(&dir, &["deb-1".to_owned(), "deb-2".to_owned()])
        ...
        .expect("stage log");
        assert!(log.contains("runtime_acls=yes"), "{log}");
    ...
    #[test]
    fn runtime_acls_witness_write_failure_is_propagated() {
        let dir = std::env::temp_dir().join(format!(
            "rtacls_witness_blocker_{}_{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .expect("clock")
        ...
        let err = write_runtime_acls_witness(&dir, &["deb-1".to_owned()])
