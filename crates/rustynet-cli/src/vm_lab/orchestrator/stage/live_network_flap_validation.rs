...
impl OrchestrationStage for LiveNetworkFlapValidationStage {
    fn id(&self) -> StageId {
    ...
    fn execute(&self, ctx: &mut OrchestrationContext) -> StageOutcome {
        ...
        match result {
            ...
            Err(e) => {
                StageOutcome::Failed(format!("live_network_flap binary invocation failed: {e}"))
            }
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn stage_id_is_live_network_flap() {
        let stage = LiveNetworkFlapValidationStage;
        assert_eq!(stage.id(), StageId::LiveNetworkFlapValidation);
    }

    #[test]
    fn stage_name_is_lowercase_kebab() {
        let stage = LiveNetworkFlapValidationStage;
        assert_eq!(stage.name(), "live_network_flap_validation");
    }

    #[test]
    fn depends_on_live_managed_dns() {
        let stage = LiveNetworkFlapValidationStage;
        assert_eq!(stage.dependencies(), &[StageId::LiveManagedDnsValidation]);
    }
