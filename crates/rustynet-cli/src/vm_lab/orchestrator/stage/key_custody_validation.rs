...
impl OrchestrationStage for KeyCustodyValidationStage {
    ...
    fn execute(&self, ctx: &mut OrchestrationContext) -> StageOutcome {
        ...
        // Aliases whose key-custody validator actually ran and passed — the
        ...
        for alias in &aliases {
            ...
            match adapter.run_role_validator(RoleValidatorKind::KeyCustody, expected_node_id, None)
            {
        ...
        // failure fails the stage — an unwitnessed key-custody pass must
        // never be recorded (the runner would demote it).
        match outcome_for(&failures, &reported_skips) {
            StageOutcome::Passed => match write_key_custody_witness(&ctx.report_dir, &validated) {
                Ok(()) => StageOutcome::Passed,
                Err(e) => StageOutcome::Failed(format!("key custody witness write failed: {e}")),
            },
            other => other,
        }
    }
...
/// Writes the QH-83 witness line for a key-custody PASS: the validated node
/// count plus each alias whose custody contract was actually proven, so a
/// bare PASS can never be recorded without the on-disk evidence behind it.
/// An unwritable witness fails the stage — fail closed.
fn write_key_custody_witness(
    report_dir: &std::path::Path,
    validated: &[String],
) -> Result<(), String> {
    append_stage_evidence_line(
        report_dir,
        StageId::KeyCustodyValidation.as_str(),
        &format!(
            "key_custody=yes validated_nodes={} ({})",
            validated.len(),
            validated.join(",")
        ),
    )
...
mod tests {
    ...
    fn empty_assignments_is_skipped_never_passed() {
        ...
        assert!(
            matches!(
                KeyCustodyValidationStage.execute(&mut ctx),
                StageOutcome::Skipped(_)
            ),
    ...
    // best-effort.
    #[test]
    fn key_custody_witness_line_names_the_validated_node_count() {
        ...
        write_key_custody_witness(&dir, &["deb-1".to_owned(), "deb-2".to_owned()])
        ...
        .expect("stage log");
        assert!(log.contains("key_custody=yes"), "{log}");
    ...
    #[test]
    fn key_custody_witness_write_failure_is_propagated() {
        let dir = std::env::temp_dir().join(format!(
            "keyc_witness_blocker_{}_{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .expect("clock")
        ...
        let err = write_key_custody_witness(&dir, &["deb-1".to_owned()])
