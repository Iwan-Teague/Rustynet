...
impl OrchestrationStage for LiveManagedDnsValidationStage {
    ...
    fn execute(&self, ctx: &mut OrchestrationContext) -> StageOutcome {
        if let Some(skip) =
            crate::vm_lab::orchestrator::stage::exit_only_topology_skip(ctx, "managed DNS")
        {
            return skip;
        }
...
mod tests {
    ...
    // Exit-only topology → reported skip, never a hard fail (live 2026-09-10).
    #[test]
    fn exit_only_topology_is_skipped_not_failed() {
        ...
        let outcome = LiveManagedDnsValidationStage.execute(&mut ctx);
        assert!(
