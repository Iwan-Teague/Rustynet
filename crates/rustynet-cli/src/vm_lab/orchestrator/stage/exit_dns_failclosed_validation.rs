...
/// Prove every Linux exit-capable node passes the exit-mode DNS fail-closed
...
///
/// Runs after `exit_demotion_residue_validation` and before cleanup. This is
/// a per-node posture check applicable to every node regardless of role.
    ...
    fn execute(&self, ctx: &mut OrchestrationContext) -> StageOutcome {
        ...
        for alias in &aliases {
            let adapter = match ctx.adapters.get(alias.as_str()) {
                ...
                None => {
                    failures.push(format!("{alias}: no adapter for exit-dns-failclosed node"));
                    continue;
                }
            };
            ...
            if let Err(e) = validate_linux_exit_dns_failclosed(&*shell, daemon_path, alias) {
