...
use rustynet_mcp::{
    CommandOutcome, GetPromptResult, LedgerEngine, McpServer, Prompt, PromptArgument, ServerInfo,
    Tool, ToolCallResult, engine_schema_property, json_schema_array_string, json_schema_boolean,
    json_schema_object, json_schema_string, prompt_text, read_file_capped, run_server,
    run_with_timeout, spawn_logged, tail_file, text_content, tool_error, tool_success,
    ...
    fn tools(&self) -> Vec<Tool> {
        vec![
            ...
            Tool {
                ...
                description: "Start a live-lab run ON a remote host, DETACHED, and return immediately with a pid — the one action that was still a manual SSH. The orchestrator runs 30-45 min; this does NOT wait for it. It launches under setsid+nohup with all fds off the SSH channel, so the run survives the connection dropping, records its own pid for a later stop. The launcher itself no longer gates on host occupancy (QH-18): the orchestrator it starts takes a per-GUEST lock, so a second run is refused only when its guests overlap an in-flight run's, and disjoint-guest runs proceed. Poll it with host_run_status and stop it with stop_host_run. report_dir is RELATIVE to the host's repo_dir (the orchestrator refuses a non-empty one, so use a fresh name). Pass orchestrator_args as the exact flags you would give `ops vm-lab-orchestrate-live-lab` (node selectors, platform/skip flags) — each is single-quoted into the remote script, so none may contain a single quote or shell metacharacter. dry_run renders the launcher without running it. `ops vm-lab-launch-on-host`.".into(),
                input_schema: json_schema_object(
                    json!({
