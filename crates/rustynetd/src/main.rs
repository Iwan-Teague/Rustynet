...
fn parse_daemon_config(args: &[String]) -> Result<DaemonConfig, String> {
    ...
    while index < args.len() {
        match args.get(index).map(String::as_str) {
            ...
            Some("--traversal-flap-breaker") => {
                let value = args
                    .get(index + 1)
                    .ok_or_else(|| "--traversal-flap-breaker requires a value".to_owned())?;
                config.traversal_flap_breaker = match value.as_str() {
                    "true" | "1" | "yes" => true,
                    "false" | "0" | "no" => false,
                    _ => {
                        return Err(
                            "invalid traversal-flap-breaker value: expected true/false".to_owned()
                        );
                    }
                };
                index += 2;
